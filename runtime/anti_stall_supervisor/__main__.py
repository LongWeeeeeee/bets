"""CLI entrypoint: python -m runtime.anti_stall_supervisor
...
# Support both `python -m runtime.anti_stall_supervisor` (package) and
# direct path execution during staging assembly.
_PKG = Path(__file__).resolve().parent
if str(_PKG.parent) not in sys.path:
    # Ensure /root/main is on path so `runtime.*` imports resolve when needed.
    root = _PKG.parent.parent if _PKG.parent.name == "runtime" else _PKG.parent
