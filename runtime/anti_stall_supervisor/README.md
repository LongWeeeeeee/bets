...
**Location:** `/root/main/runtime/anti_stall_supervisor/`  
...
cd /root/main && /root/main/venv/bin/python -m pytest -q \
  /root/main/runtime/anti_stall_supervisor/tests/test_snapshot.py \
  /root/main/runtime/anti_stall_supervisor/tests/test_decision.py \
  /root/main/runtime/anti_stall_supervisor/tests/test_executor.py \
  /root/main/runtime/anti_stall_supervisor/tests/test_runner.py \
  /root/main/runtime/anti_stall_supervisor/tests/test_integration.py
```
...
cd /root/main && /root/main/venv/bin/python \
  /root/main/runtime/anti_stall_supervisor/__main__.py \
  --config /root/main/runtime/anti_stall_supervisor/config.json \
  --dry-run \
...
```bash
/root/main/venv/bin/python /root/main/runtime/anti_stall_supervisor/__main__.py --emit-running-tuples
```

Or use the wrapper that records before/after tuples and asserts `planned_actions=0`:

```bash
/root/main/venv/bin/python /root/main/runtime/anti_stall_supervisor/scripts/live_dry_run_validate.py
```
