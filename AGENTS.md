...
- **Деплой live pipeline:** git push `main` → на сервере `git pull` → kill старого процесса → чисти `map_id_check.txt` → рестарт. **Правка логики ставки = рестарт прода + чистка `map_id_check.txt` в том же ходу.**
