...
explorer → worker → verifier → reviewer нет. Дочерние агенты не делегируют дальше.

Общая доска — Beads. Протокол: `/Users/alex/Documents/AGENTS.md`, раздел 7.
Стартовый хук показывает каноническую БД и владельца `codex:<session_id>` или
`claude:<session_id>`. Перед работой прочти задачу и возьми её атомарно через
`bd --no-daemon --db <database> --actor <actor> update <id> --claim`.
Запиши принадлежащие задаче файлы; не правь файлы другого работающего агента.
Обновляй свою задачу по существенным результатам, закрывай только после проверки.
Доска общая для worktree; чаты и сообщения субагентов общими не становятся.
Автокоммиты всего дерева отключены. `.beads/` не синхронизировать и не публиковать.
После каждого существенного завершённого шага, включая read-only находки,
делай `bd update <id> --notes` перед долгим запуском, compaction, паузой или
ответом пользователю; checkpoint содержит workspace/branch, доказательства,
незавершённое, точное следующее действие, PID/логи и статус рестарта, свои
файлы и блокеры. Не обновляй после каждого вызова. Перед правкой прочти
preview и выполни `bd show <id> --json`. При handoff после checkpoint и остановки
правок выполни `bd update <id> --status open --assignee '' --add-label handoff`;
новый клиент проверяет cwd/Git/runtime, выполняет `bd update <id> --claim
--remove-label handoff` и снимает handoff. Возраст сам по себе не даёт забрать claim.

Claim закрепляет задачу и её файлы, а не весь проект или доску. Другой агент,
в том числе из другого harness, может без claim дописать находку через
`bd --no-daemon --db <db> --actor <actor> comments add <id> --author <actor> -f <file>`
или создать отдельную незанятую карточку `kb`. Указывай находку, доказательства,
неопределённость и следующий шаг; не перезаписывай чужие notes/status/assignee.
Вклад комментарием не требует отдельной задачи для checkpoint. Владелец перед
продолжением и закрытием читает `bd comments <id> --json` и учитывает находки в
своём checkpoint. Claim чужой задачи для комментария не нужен.
