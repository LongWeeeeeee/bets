...
area: ml
status: full
corpus: "482 486 про-карт; исправленный forward-протокол E-96: три непересекающихся окна после 26.05.2026, 13 653 OOF-карты после исключения 1 984 строк с repeated timestamp; pickBans pilot — 5 620 карт / 2 248 rolling OOF"
verdict: "Цель во всех новых прогонах — didRadiantWin, kills не используются и не фильтруются. Исправленная полная предматчевая модель: 0.7121 AUC; независимая проверка 31/31 PASS. Новый блок качества из GPM/LH/IMP даёт +0.0112 к старой E96, но ровно поглощается полной E99: −0.0002 [−0.0008,+0.0005]. Найден один новый небольшой инкремент: исторические level/denies игрока относительно нормы позиции/героя дают 0.7121 -> 0.7133, +0.0013 [95% ДИ +0.0001,+0.0025], положительно во всех трёх окнах; top-25 WR 81.0% -> 81.6%. Это диагностическое подтверждение после exploratory-подсказки, нужен будущий untouched holdout. Если прогноз делается после старта карты, крупнейший уже найденный резерв — E-134: live state 0.8126 -> 0.8306 с prematch prior, а на равных картах прибавка +0.042…+0.074. На удвоенном pickBans snapshot три pilot остались нулевыми: порядок −0.0026, сигнатурные баны +0.0002, reactive counter +0.0007; полный сбор продолжается на serv1. Главный незакрытый prematch-слой — exact production HybridPlayerRosterElo: попытка покрыла только 97.07% корпуса и не была оценена. Serv2 недоступен по сети"
...
На serv1 собран snapshot 5 620 карт; 10 picks и их порядок валидны у всех.
Это почти вдвое больше первого прогона на 2 892 картах. Пилоты используют
rolling OOF только по более ранним собранным картам:

| группа | rolling OOF | ΔAUC к той же recalibrated quality-базе | 95% ДИ |
|---|---:|---:|---:|
| простой pick order / information advantage | 2 248 | −0.0026 | [−0.0054,+0.0002] |
| bans сигнатурных/привычных героев | 2 219 | +0.0002 | [−0.0027,+0.0031] |
| counter-quality только по уже видимым picks | 2 248 | +0.0007 | [−0.0026,+0.0039] |

Ни одна группа не отделилась от нуля и после удвоения snapshot. Это всё ещё не
доказательство отсутствия сигнала у более сложных stage-aware конструкций:
период короткий, а сами три модели линейные. Диагностически на первом snapshot
`pos1_rank_diff` имел raw AUC 0.5304, reactive pos1 — около 0.542, но направление
просмотрено post-hoc и не считается открытием. Collector на serv1 жив и
продолжит после сброса часовой квоты Stratz.
...
- **serv1:** независимая проверка quality OOF выполнена ранее; сейчас живой
  многопоточный collector `pickBans` (5 620 строк, ждёт сброс часовой квоты);
