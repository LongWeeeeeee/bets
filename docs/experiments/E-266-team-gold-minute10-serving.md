...
После warmup настоящий PID397023 загрузил модель с тем же SHA. В map_verdicts подтверждены 4 реальные карточки с обеими строками и нужным порядком, например Top lose62%,Mid lose54%,Bot lose64% → ML Laning Dire78.7% (золото,10мин); EarlyWin Radiant53.9% → All Dire50.9%. Это проверка вывода, не исходов или качества. Evidence: `runtime/artifacts/laning/20260909_team_nw10_v1/runtime_activation.json`.
