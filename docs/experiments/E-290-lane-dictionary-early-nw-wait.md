...
**Последняя цель владельца: командный NW на10:00 >=1500; приемлемая частота65%.**
Рекомендация по дополнительной сетке —4:00/+1300:70.43% primary,68.06% clock−1.
Это рекомендация, новый деплой не выполнялся.
...
### Дополнение: владелец принимает частоту65%, выбор простого порога

**OBSERVED.** Для той же цели signed teamNW10>=1500 проверен постоянный entry
1000–1500 шаг100 с4-й до9-й минуты, первое пересечение. Confirmation reused.

| Entry с4-й минуты | Primary hits/n | Частота | Wilson95CI | Охват | Clock−1 частота |
|---|---:|---:|---|---:|---:|
| 1000 | 275/470 | 58.51% | 54.00–62.88% | 33.26% | 56.43% |
| 1100 | 272/431 | 63.11% | 58.46–67.53% | 30.50% | 60.73% |
| 1200 | 269/401 | 67.08% | 62.34–71.50% | 28.38% | 64.02% |
| 1300 | 262/372 | 70.43% | 65.60–74.84% | 26.33% | 68.06% |
| 1400 | 254/344 | 73.84% | 68.95–78.20% | 24.35% | 71.43% |
| 1500 | 247/321 | 76.95% | 72.04–81.22% | 22.72% | 74.90% |

Conditional-discovery LCB65 выбирает4:900,5:1100,6/7:1200,8:1600,9:1400:
260/384=67.71%, coverage27.18%; clock−1 refit199/301=66.11%.

**DERIVED/INFERRED.** Рекомендация lead — постоянные4:00/+1300 до fallback10.
Это минимальный проверенный постоянный порог, у которого наблюдаемая частота
выше65% при обеих clock-конвенциях:70.43% и68.06%. Discovery73.24%/72.14%.
Порог1200 даёт67.08% в primary, но64.02% в alternate; поэтому запаса недостаточно.
Переменное расписание не показывает убедительного выигрыша для своей сложности.
Если требовать нижнюю Wilson95-границу>=65% сразу в обеих конвенциях,
первый прошедший на этой сетке —1400. Это дополнительное требование, которого
владелец не ставил; даже такие интервалы не гарантируют будущую точность.

**NOT_CHECKED.** Новая проспективная выборка и live подтверждение. Выбор учитывает
уже многократно просмотренный confirmation и является исследовательским решением.
Вопрос «что ставим» трактован как запрос рекомендации; новый деплой не выполнялся.

Харнесс и воспроизводимая команда (артефакт write-once, при повторе нужен новый dest):

```sh
/usr/bin/nice -n 19 venv_catboost/bin/python3 runtime/experiments/star-dispatch/lane_target1500_accept65.py
```

Результат `runtime/artifacts/star-dispatch/lane_wait_20260914/target1500/accept65.json`
содержит SHA256 харнесса и ссылку на frozen manifest. Exit0 за0.31s; frozen hashes
и независимые scalar counts для всех14 правил в обоих split совпали.
Где искать ошибку: принять65% за вероятность выигрыша карты; заменить первое
пересечение отдельным минутным срезом; выбрать по primary и забыть clock−1;
представить эмпирическое превышение65% как гарантированную нижнюю границу.
...
```orchestra-evidence-v1
{
  ...
  "sources": [
    ...
    {
      "id": "ACCEPT65",
      "path": "runtime/artifacts/star-dispatch/lane_wait_20260914/target1500/accept65.json",
      "sha256": "d5d37f4ac4668cbb2b8f8bf611be209707eb1ca6cab1609f515a43dfe3dd9044"
    }
  ],
  "claims": [
    {
    ...
    },
    {
      "id": "F9",
      "kind": "OBSERVED",
      "claim": "Fixed1300 from4: primary262/372=70.43%, alternate211/310=68.06%. Fixed1200 alternate226/353=64.02%.",
      "sources": [
        "ACCEPT65"
      ],
      "scope": "Reused retrospective confirmation; lead recommends1300 for empirical65 acceptance, no rollout."
    }
  ],
  "checks": [
    {
