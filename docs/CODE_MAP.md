...
`rad_ge30`, `dire_ge30`, `total_ge55` карточка показывает их через `ml_panel.render` после окон
и перед `dur43`, без блока E281. При отсутствии хотя бы одного, `A_fallback` или ошибке панели
