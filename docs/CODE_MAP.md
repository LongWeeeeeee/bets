...
3. **OpenDota по `match_id`** (`_winline_fetch_map_winner` → `radiant_win`; тот
   же источник закрывает серии в живом ELO). Только он знает исход ПОСЛЕДНЕЙ
   карты серии, когда серия ушла из живых списков, и только ему нужен `match_id`
   — поэтому первые два источника работают и для опроса, заведённого в перерыве
   без id матча.
...
(`_winline_flush_pending_map_winners`). Цена ошибки уже была заплачена дважды:
03.09.2026 во время аварии api.opendota.com (522 и HTTP=000 с двух независимых
сетей при живых winline.ru и example.com) карта 1 PuckChamp — Team Spirit
Academy ушла без победителя, хотя Steam в тот же момент отдавал по лиге 19944
`wins 0:1`.
