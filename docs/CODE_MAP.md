...
**Победитель карты.** В `🏁` добавляется строка `🏆 победа: <команда>`. Источник —
OpenDota по `match_id` карты (`_winline_fetch_map_winner` → `radiant_win`; тот же
источник закрывает серии в живом ELO). Синхронного источника победителя в момент
конца карты нет, поэтому факт конца карты не задерживается: если матча в OpenDota
ещё нет, `🏁` уходит без строки, а имя догоняет отдельным сообщением `🏆`
