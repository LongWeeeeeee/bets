...
Рынок окна закрывается **за 2 минуты до его старта**, поэтому полоса отправки — последние минуты перед дедлайном (позже = больше живых данных):

| окно | полоса (игровое время) | дедлайн рынка | `min_ed` | второй гейт | WR @1.8 (все / поздние 30%) |
|---|---|---|---|---|---|
| `5_15` | 02:00–03:00 | 03:00 | 0.3 | NW-лид ≥ 0 **и** `lane_kills_adv_dict` ≥ 0.2 того же знака | 64.5% / 61.0% |
| `10_20` | 06:00–08:00 | 08:00 | 0.2 | NW-лид таргета ≥ +500 | 74.6% / 72.7% |
| `15_25` | 11:00–13:00 | 13:00 | 0.2 | NW-лид таргета ≥ +1000 | 72.2% / 70.9% |
| `20_30` | 16:00–18:00 | 18:00 | 0.2 | NW-лид таргета ≥ +1500 | 84.3% / 78.7% |

Ресёрч 2026-08-04 (1425 Tier-1 карт 7.41+, `runtime/experiments/kills/kills_window_*.py`): ничья в окне = луз, безубыток при 1.8 = 55.56%. Перенос отправки к дедлайну поднял WR сквозной политики с 69.3% до 73.5%, вместе с поднятыми NW-порогами — до 77.5% (на отложенных по времени 30%: 66.4% → 75.5%). `min_ed` не менялся: после NW-гейта он не узкое место.
