...
| `BET_SHOW_DRAFT_BLOCKS` | `0` | `1` возвращает в карту ставки подробные драфт-блоки, скрытые 20.09.2026: `Lanes:` (обёртка `_build_lane_block_for_bet`, 8 точек сборки), `Early NW (20-28):`/`Early Winner (20-28):`/`Late: (28-60 min):`/`All:`/`Mix:` (обёртка `_compose_star_metric_blocks_for_bet`, 4 точки) и `Kills_window:` внутри `_format_live_message_state_block`. Чистые форматтеры `_build_lane_block`/`_compose_star_metric_blocks_for_message`/`_format_kills_window_values_block` не менялись. Читается при каждой сборке (`_bet_show_draft_blocks`), без рестарта не применяется — процесс читает env при старте systemd |
...
в manifest.json/results.json → дата в имени каталога/родителя/деда → новейший mtime `*.joblib|*.npz|*.cbm` → `None`
