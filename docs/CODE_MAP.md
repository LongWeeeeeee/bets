...
- `base/kv3_panel_serving.py` — боевая подмена 6 вердиктов панели (w_5_15, w_10_20, w_15_25, w_20_30, rad_30_25,
  ...
  `EXTRA_TARGETS=(dire_30_25,rad_ge30,dire_ge30,total_ge55)` добавляет вердикты B без аналога A
  только при наличии записи в `panel.json` и обоих файлов модели/калибровки. Plain-цели:
...
- `base/series_tempo.py` — журналируемая тень E-331 для целей в `TEMPO_CONSTANTS` (пока только
  ...
  `SERIES_TEMPO_APPLY=1` (по умолчанию) применяет поправку к вероятности B указанных целей до
...
| `_deliver_and_persist_signal(...)` | 27788 | единая точка доставки; half-stake ELO, согласие предматчевой ML и `late_model_against` / `late_model_target_unknown` (любая обычная ставка, если Late ML против target или сторону ставки взять неоткуда) — до prepare/dedup. Отказ любого из этих гейтов пишет `kind=reject` + `bet_message` в журнал вердиктов (хвост `tail_log`), матч не закрывает |
| `_record_map_verdict(match_key, *, verdict, kind, reason, metrics, protracker, star, elo, identity, dispatch, extra, create_only, bet_message, kills_window, snapshot_only)` | ~27374 | журнал вердиктов карт: upsert 1 карта = 1 запись в `MAP_VERDICTS_PATH`; verdicts копятся (склейка подряд-дублей, cap 50), metrics/protracker/star/elo/kills_window заменяются последним снапшотом; `bet_message` хранит последний текст ставки и не затирается пустым; `snapshot_only=True` обновляет снапшот без новой строки вердикта; никогда не бросает; вызывается из всех ✅/⚠️/⛔ ВЕРДИКТ-точек `check_head`, kills-диспетчеров, delayed worker и гейтов `_deliver_and_persist_signal` |
| `_flush_map_analysis_snapshot(match_key, ctx)` | ~27485 | пишет metrics/star/elo/bet_message из `_verdict_ctx` в журнал без вердикта; нужен `tail_log`, когда разбор уже есть, а send/reject ещё не вызывали `_verdict_print` |
...
`register_live_map_context(...)` принимает необязательный `observed_game_time` (секунды, отрицательное в драфте). В sourcetv-режиме доигранная карта ~15 мин видна под новым ключом `dltv.org/matches/<id>.<kills>` с тем же alias match_id серии (E-294 follow-up: 29/162 серий в проде применялись дважды). Если этот match_id уже применён этой серией и `observed_game_time >= LIVE_ELO_ALIAS_DUPLICATE_MIN_GAME_TIME_SECONDS` (константа модуля, default 600 c, env `LIVE_ELO_ALIAS_DUP_MIN_GAME_TIME`) — карта не ставится в очередь: `current_map_already_applied=True`, `duplicate_reason="alias_finished_map"`. `cyberscore_try.py::_register_completed_live_map_for_elo` пробрасывает `observed_game_time=game_time` и логирует `duplicate_reason` в строке `LIVE_ELO_NOT_APPLIED`.

#### Основной командный ELO: вариант A с 25.09.2026 (E-338), K24 — откат
С 25.09.2026 `get_matchup_summary` отдаёт `ELO/models.py::a_lineup_summary` (source=`elo_composition_a`, в Telegram «ELO состава (A):»). Обновление A отличается от K24 тремя множителями: длительность карты `clip(1981/max(dur,600),0.6,1.6)`, затухание по перевесу `2.2/(2.2+0.001·max(edge,−1000))` и ускорение новичков `1+2/(1+games/30)`. Состояние A (`player_a`, счётчик карт, 14 дней истории) лежит в том же снимке рядом с K24. Env `ELO_SERVED_COMPOSITION=a|k24` (по умолчанию `a`, `k24` — откат). K24 с одним предупреждением в логе отдаётся и при неверном значении, и когда в снимке нет A-состояния. Живая длительность: `stratz_map_result` (`durationSeconds`) → `cyberscore_try.py::_live_elo_winner_lookup(..., with_duration=True)` → `_build_live_applied_update(..., duration_seconds=)`. На пути «исход по сдвигу счёта» длительность берётся только из дискового кэша Stratz, без сети; без длительности множитель 1. Сначала выкатывается код, потом снимок: старый код отвергает A-снимок. Все потребители (Telegram, правила диспетчера, kills25-shadow) читают одну отданную композицию; признаки предматчевой модели остаются на `prematch_lineup_summary`. Контракт: [вариант A](elo-variant-a.md). Ниже описан K24, чьи правила доступности и покрытия A повторяет.

`ELO/models.py::k24_lineup_summary` — основной расчёт для Telegram и правил: среднее независимых `player_k24` пяти игроков, начальный рейтинг 1500, K=24, шкала 400. Обновление происходит только по завершённым картам, без tier-поправок, decay и бонусов hybrid. `prematch_lineup_summary` сохраняет обучающий контракт E-173 для замороженного признака ML `hybrid_strength`: `team_id=None`, имя команды, `TIER3`, сортировка account ID вместе с позициями, полный `team_strength`, разница сил /400. Веса и прочие признаки ML не меняются.
...
В Telegram основной блок называется «ELO состава (K24)», source=`elo_composition_k24`; старый source hybrid сохраняет прежнюю подпись для совместимости. Признаки `elo`/`opp_elo` внутри обученной модели и диагностический `last_model_elo` сохранены. K24 хранит 14 дней обратимых обновлений: запрос на начало карты исключает результаты с временем завершения >= этому началу; вне покрытия возвращает `None`. Старый снимок без K24-схемы требует явной offline-пересборки. Array sidecar и live delta версионированы; повреждённый выбранный live-state не заменяется молча базовым рейтингом. Ранги команд строятся по K24 последних известных составов, без подстановки будущего snapshot-ранга в исторический запрос. Контракт и миграция: [K24](elo-k24-primary.md).

K24 вставляет `replay_late` внутрь сохранённой истории и пересчитывает последующие K24-обновления; hybrid сохраняет прежнюю семантику. Индекс membership снимка охватывает 14 дней, срок удержания tombstone остаётся 7 дней. Если дата вне покрытия или исходы последующих обновлений недоступны, `rebase_runtime_model_state` останавливается до записи состояния и progress. Эта проверка действует и при `LIVE_ELO_REBASE_STRICT_ORDER=0`.

Живые результаты и ожидающие карты сохраняются при смене ELO-среза через `rebase_runtime_model_state` и replayable progress; CLI принимает `--snapshot`, `--state`, `--progress`. Точное покрытие свежих результатов хранится в `meta.recent_completed_match_ids`/`recent_completed_match_ids_coverage_since`; неоднозначный перенос останавливается до записи. Ночная цепочка заменяет подготовленный снимок только после успешной перебазировки. Tombstone, доказанный fallback'ом по паре команд, хранит `snapshot_covered_match_id` (ключ снимка, которым закрыт); если следующая перебазировка не может подтвердить его новым снимком, tombstone не отбрасывается с ошибкой, а остаётся до истечения `SNAPSHOT_RECENT_RESULT_COVERAGE_SECONDS` (7 дней) со строкой в stderr «snapshot tombstone … не подтверждён новым снимком». Карта ledger с обеими известными базе командами, отсутствующая в корпусе снимков и с `result_ts <= cutoff`, больше НЕ отклоняется по умолчанию (bbd0664f/04b8252c): она replay'ится после среза с предупреждением в stderr и действием `replay_late` («… replay после среза, порядок ELO-событий нарушен»), потому что отказ не восстанавливает порядок, а лишь замораживает прод на устаревшей базе; итоговая строка — «[ELO] rebase: N live-карт(а) отсутствуют в снимке и replay'нуты после среза». Env `LIVE_ELO_REBASE_STRICT_ORDER=1` возвращает старый жёсткий отказ (`RuntimeRebaseError`). Отдельно `rebase_runtime_model_state` схлопывает «alias-твинов» при replay: две записи ledger с одним `match_id` (>0), той же неупорядоченной парой команд и `result_timestamp` в пределах `LIVE_ELO_ALIAS_TWIN_WINDOW_SECONDS` (default 600 c, env `LIVE_ELO_ALIAS_TWIN_WINDOW`) применяются к модели один раз — сама запись в `rebased_applied` остаётся, пропускается только повторное `model.process_match`; строка — «[ELO] rebase: N дубликат(ов) alias-карт пропущено». Отдельный `ELO/recover_live_results.py` восстанавливает подтверждённые пропущенные события в отдельные файлы состояния/progress и отчёт; `--verify-current-delta` сверяет контрольный replay текущих результатов с действующей дельтой. Контракт CLI и инцидент: [сохранение живых результатов](elo-live-rebase.md).
...
| `BET_REQUIRE_LATE_WIN_MODEL` | `1` | `0` снимает гейт late-модели (`_late_win_model_reject_for_delivery`). `late_model_against` — любая обычная ставка, если строка `🕑 Late ML-модель` называет не target. `late_model_missing` — только late-driven: без строки late-ставки нет. `late_model_target_unknown` — модель назвала сторону, а сторону ставки взять неоткуда (нет `stake_multiplier_context`): запрет, а не молчаливый пропуск (01.09.2026) |
...
| `BET_SHOW_DRAFT_BLOCKS` | `0` | `1` возвращает в карту ставки подробные драфт-блоки, скрытые 20.09.2026: `Lanes:` (обёртка `_build_lane_block_for_bet`, 8 точек сборки; в скрытом режиме уходят только заголовок и Top/Mid/Bot, а `ML Laning: …`, `lane_adv_dict: …`, lane-kills и `Lane_adv_protracker: …` печатаются на прежнем месте), `Early NW (20-28):`/`Early Winner (20-28):`/`Late: (28-60 min):`/`All:`/`Mix:` (обёртка `_compose_star_metric_blocks_for_bet`, 4 точки) и `Kills_window:` внутри `_format_live_message_state_block`. Чистые форматтеры `_build_lane_block`/`_compose_star_metric_blocks_for_message`/`_format_kills_window_values_block` не менялись. Читается при каждой сборке (`_bet_show_draft_blocks`), без рестарта не применяется — процесс читает env при старте systemd |
...
| `DLTV_ROLE_LOOKUP` | `1`; `0` — sourcetv-проба не запрашивает роли DLTV, перебор позиций как до E-333 (откат) |
| `DLTV_ROLE_WEIGHT` | `0.2` — вес совпадения с ролью DLTV в переборе позиций пробы (история = 1.0) |
| `DLTV_ROLE_LOOKUP_DEADLINE_S` | `6` — общий дедлайн запросов ролей DLTV на один перебор |
| `DLTV_ROLE_CACHE_PATH` | `{PROJECT_ROOT}/runtime/dltv_player_roles_cache.json` — кэш ролей DLTV (атомарная запись) |
...
### Карта ставки: ML-строки с припиской свежести данных (20.09.2026)

Вместо драфт-блоков (см. `BET_SHOW_DRAFT_BLOCKS`) каждая ML-строка карты несёт хвост
`| данные до DD.MM (N дн.)` — дата UTC самых свежих данных за моделью и её возраст.
Формат собирает `base/model_data_asof.py` (stdlib-only): `data_asof_ts(model_dir)` →
`freshness_note(ts)` → `model_dir_note(model_dir)`. Заголовок Early NW переименован:
`🕐 Early NW ML-модель (20–28 мин)`.

| Строка | Источник даты | Где ставится |
|---|---|---|
...
(приписки нет). Хвост ставится ПОСЛЕ процента/★ — регексы гейтов `_WIN_MODEL_PANEL_RE` и `_LATE_WIN_MODEL_PANEL_RE`
матчат только префикс строки до стороны и не затрагиваются. Тесты: `base/tests/test_model_data_asof.py`,
`base/tests/test_bet_card_draft_blocks_hidden.py`.
...
| `_lane_stats_to_counts` | 6418 | воронка каскада линий: ячейка → `(wins, draws, losses, games)`. При `LANE_CELL_VALUE != label` пересобирает первые три из блока `nw10_*` (`nw_share` — доля матчей с лидом по золоту, `nw_mean` — обрезанное среднее через `tanh(mean/LANE_CELL_NW_SCALE)`), НЕ меняя `games`: число игр решает, какой слой каскада сработает, и должно остаться прежним. Дефолт `label` = прежнее поведение |
| `calculate_lane_nw_advantage` | 9800 | `calculate_lane_nw_advantage(radiant_heroes_and_pos, dire_heroes_and_pos, heroes_data)` — тот же каскад, но по колонкам `nw10_*`: ожидаемый перевес Radiant по золоту на 10-й минуте. Env: `LANE_NW_USE_CLIP` (1 = читать `nw10_clip_sum`, 0 = сырую сумму), `LANE_NW_MIN_GAMES`, `LANE_NW_RELIABILITY_PRIOR` (по умолчанию наследуют значения `LANE_KILLS_*`). Обе функции — обёртки над `_calculate_lane_metric_advantage(..., prefix=...)` |
...
- Поля одного lane-ключа: `kills10_leads`, `kills10_draws`, `kills10_games`, `kills10_diff_sum`, `kills10_diff_sq_sum` и с 12.08 такой же блок по золоту — `nw10_leads`, `nw10_draws`, `nw10_games`, `nw10_diff_sum`, `nw10_diff_sq_sum`, `nw10_clip_sum` (`NW@10 = radiantNetworthLeads[10]`, обрезка выбросов на `ANALISE_LANE_NW10_CLIP`, по умолчанию ±3000). Отдельного набора дублирующихся ключей и отдельных kills/nw-словарей нет.
- `base/explore_database.py` is sqlite-first: early/late/post_lane primary artifacts are `early_dict_raw.sqlite3`, `late_dict_raw.sqlite3`, `post_lane_dict_raw.sqlite3` with `kv(key TEXT PRIMARY KEY, value BLOB)` + `meta`, format_version `1`/backend `sqlite_kv`; lane primary artifact is `lane_dict_raw.sqlite3` with `stats` + `meta` both `WITHOUT ROWID`, format_version `2`/backend `sqlite_stats`; `stats` has `key`, `wins/draws/games`, five `kills10_*` fields and six `nw10_*` fields. Live `cyberscore_try._load_stats_dicts` opens lane sqlite through `_prepare_indexed_stats_lookup` (тот же `_SqliteStatsLookup`, что у early/late/post_lane) и оборачивает его в `functions._StructuredLaneLookup` — точечные `.get()` без материализации 7M ключей. `cyberscore_try._load_lane_dict_from_source` остаётся offline/test-путём: SELECT по фактическому `PRAGMA table_info(stats)` в полный dict (nw10-колонки — только при `LANE_CELL_VALUE != label`).
...
**Правило допуска в post_lane (all) — с 2026-08-09:** `POST_LANE_MIN_DURATION = 20` (env `ANALISE_POST_LANE_MIN_DURATION`). Порог 28, выбранный 07.08, откачен в E-65: он подбирался на конструкции, которой больше нет (до разделения популяций early/early_end), а на собственной популяции post_lane 20 не хуже. Гейт 10-й минуты (`ANALISE_POST_LANE_MINUTE10_GATE_ENABLED`) остаётся **выключенным**: включение стоит 3.4–7.3 п.п. и режет словарь вчетверо (E-65).

**Пороги ветвления lane-читателя (`functions.calculate_lanes`, все — env через `_lane_min_games_env`), 2026-08-09:**

| константа | default | что делает |
|---|---|---|
| `LANE_2V2_MIN_GAMES` | `30` | было `2`; при двух играх ветка срабатывала на 37.4% боковых линий и стоила 22 п.п. (E-66). Порог обслуживает и пятый член ансамбля в `counterpick_lanes` |
| `LANE_2V1_MIN_GAMES` | `10 ** 9` | ветка 2v1/1v2 **выключена**: одиночный специфичный ключ проигрывает среднему четырёх пар 1v1 при любом объёме ячейки |
| `LANE_MID_MIN_GAMES` | `20` | **только средняя линия**. Раньше mid читал `LANE_2V1_MIN_GAMES`, из-за чего отключение боковой ветки обнуляло mid целиком |
| `LANE_1V1_MIN_GAMES` | `50` | пары в `counterpick_lanes` |
| `LANE_SYNERGY_MIN_GAMES` / `LANE_SOLO_MIN_GAMES` | `30` / `10` | не калибровались |

`counterpick_lanes` усредняет четыре пары 1v1 **плюс** ячейку 2v2 пятым членом (не заменой): при 30+ играх это 73.2% против 72.1% у чистой четвёрки. Правка живёт в читателе и не зависит от пересборки словаря.
...
- `STAR_SIGNAL_METRICS = {'counterpick_1vs1', 'counterpick_1vs2', 'dota2protracker_cp1vs1', 'solo', 'dltv_rating', 'dota2protracker_solo', 'dota2protracker_duo', 'pos1_vs_pos1'}` (frozenset; `synergy_duo`/`synergy_trio` сюда не входят).
- `pos1_vs_pos1` — звёздная метрика с 11.08 (E-71). Пороги заданы ТОЛЬКО для `early_output` (7/11/15 на WR60/65/70) и `mid_output` (8 на WR60); в `all_output` порога нет, и метрика там звёзд не даёт. Выше WR60 для late порога нет намеренно: на популяции `is_late_match` метрика сильна в середине (|v|>=8 — 58.8%) и выдыхается к хвосту (|v|>=14 — 50.8%).
...
`ML_PANEL_KILLS_DISPLAY=b` (по умолчанию): при трёх вердиктах `B_kv3`
`rad_ge30`, `dire_ge30`, `total_ge55` карточка показывает `P(≥30)`/`P(≥55)` из B с одним десятичным знаком под заголовком «Килы ML · B» после окон
и перед `dur43`; окна и длительность идут через `ml_panel.render`, без блока E281. При отсутствии хотя бы одного, `A_fallback` или ошибке панели
остаётся блок E281. `ML_PANEL_KILLS_DISPLAY=band` возвращает прежний B-блок с `rad_30_25`,
опциональным `dire_30_25` и `total_55_50`; `e281` принудительно показывает E281.
...
**Источник live-кэфов Winline — дотовская страница (с 05.08.2026).**
`BOOKMAKER_URLS["live"]["winline"] = https://winline.ru/stavki/sport/kibersport/dota_2`
вместо общего `/kibersport/live`. Общий фид рендерит ленту порциями (~9 блоков) во
ВНУТРЕННЕМ контейнере `div.main__wrapper`; `document.body` там не скроллится вовсе
(`scrollHeight == innerHeight`), поэтому все `window.scrollTo(...)` в парсере — no-op, и
матч ниже первой порции отсутствовал в DOM: поллер отдавал `match not found in rendered
DOM/network payload` при живой карточке на сайте (05.08.2026, `No Hoodwink — Lynx`,
65 промахов подряд). На `dota_2` вся дотовская лента попадает в DOM без прокрутки.
Страховка на случай длинной ленты — `_sweep_camoufox_feed` (прокрутка внутреннего
контейнера, `WINLINE_FEED_SWEEP_STEPS`/`_PAUSE_SECONDS`/`_MIN_INTERVAL_SECONDS`):
выполняется после каждой навигации в acquisition-режимах и один раз на промах пары
(с троттлингом по странице), после чего снимок перечитывается.

**Заголовок дисциплины и порядок сторон (с 05.08.2026).** Закреплённая карточка
сверху страницы пишет шапку через ЗАПЯТУЮ (`DOTA 2, 1w Essence`), а лента — через `|`.
Вариант с запятой не был границей карточки, поэтому название турнира попадало внутрь
текста события: токен `1w` лиги `1w Essence` встречался раньше `TEAM LIQUID`, и
`_winline_team_order` возвращал `reverse` — цены уезжали не на ту команду (05.08.2026
`Team Liquid` при +6158 нетворса ушёл в сообщение аутсайдером 3.13 против 1.33).
`_winline_strip_discipline_header` срезает ведущую шапку до первого «капсового» токена
(название турнира набрано не капсом), и порядок считается уже по названиям команд.

**Отсечка линии (`_looks_future_context`).** На дотовской странице live-карточки
соседствуют с линией (`Завтра 15:00`), у которой ЕСТЬ рынок `1 карта` с кэфами. Поэтому
доказательством live считаются только СИЛЬНЫЕ маркеры статуса игры
(`_LIVE_STRONG_MARKER_RE`: слитное `2карта`, счётчик `2К`, таймер `18'`, счёт в скобках);
название рынка `1 карта` (с пробелом) и любое `\d+:\d+` — слабые (`_LIVE_WEAK_MARKER_RE`)
и часы старта (`_PREMATCH_CLOCK_RE`) не отменяют. Тесты — `base/tests/test_winline_dota_feed_source.py`.
...
**Стоимость опроса (12.09.2026).** `_winline_fast_snapshot` читает HTML в общем
потоке Camoufox; `_winline_fast_collect_from_payload` разбирает его уже в потоке
вызывающего. `_WinlineDOMSnapshot` хранит один BeautifulSoup и тексты узлов на
снимок; в пределах одного batch они повторно используются для нескольких карт.
Новый DOM, раскрытие закреплённой карточки и физический reload заменяют снимок.
Границы карточек, порядок команд и проверка кнопок исходов остаются общими с
полным парсером. Полный fallback и навигация по-прежнему выполняются в Camoufox.
Обзорная страница остаётся отдельной: HTML и body text берутся одним DOM RPC;
при ошибке или неполном ответе сохраняются `page.content()` и чтение body.
При наличии body text повторного BeautifulSoup нет; запасной разбор HTML
выполняется после освобождения worker. `_WinlineDOMSnapshot` также хранит
планы поиска имён и результаты сопоставления текста с парой команд; срок жизни
кэша равен одному снимку. Готовый контекст карточки не кэшируется. Выбор pinned
карточки читает список одним RPC, затем повторно проверяет имя и выделение
живой карточки перед кликом; недоступный batch RPC использует прежний обход.

Уведомления Winline отправляет один daemon-поток `winline-notifications`.
Исчезновение карточки из свежего обзора останавливает её опрос с явным
`proven=False`: это не подтверждение окончания карты. После обычного окна
ожидания разрешено только «опрос остановлен, конец карты не подтверждён».
У live-карточек разрешённых лиг DLTv-часы обновляются и без map-рядов с ценами
(`_winline_card_dltv_draft_notify(clock_only=True)`); этот путь не создаёт
опросы и не отправляет драфты. В сообщениях terminal/stopped/winner время
подписано «последнее известное время», золото — «по последним данным»:
сохранённый перевес не объявляется победителем или подтверждённой финальной
статистикой (инцидент Yangon Galacticos — YACHE123, 15.09.2026).
Tick кладёт копию уже ориентированного payload в очередь; Telegram и поиск
победителя не удерживают поток опроса. Последовательные цены одной карты с тем
же статусом могут объединяться; переходы статуса и терминалы сохраняют порядок.
Конец карты обрабатывается до следующего сообщения, включая до трёх повторов
при отказе доставки. После исчерпания повторов очередь продолжает работу.
Лимит 256 — мягкий: терминалы и котировка, необходимая для объявления их карты,
могут превышать его. Очередь только в памяти, не переживает перезапуск процесса.
Смена сторон переносит ключ и цены, включая завершение уже выполняющегося HTTP.
При ограничении частоты последняя цена переносится на ближайший разрешённый
момент, даже если нового poll tick нет. Более новая котировка, frozen/missing
или terminal отменяет отложенную цену. `WINLINE_ODDS_TELEGRAM_MAX_PENDING_AGE_S`
(15 с) ограничивает возраст отложенной записи с момента постановки в очередь;
после истечения она отбрасывается. Отложенная цена не блокирует другие карты.
Согласованный `card_team_order` + `card_odds` имеет приоритет над эвристикой
перестановки по прошлой цене: реальный разворот фаворита не меняет стороны.
Журнал отправок добавляет `observation`: идентификатор попытки/архива, исходную
и итоговую пару, основание ориентации, время чтения DOM, очереди, начала worker
и начала/окончания отправки. Это не время изменения цены у букмекера и не
подтверждение показа сообщения на устройстве пользователя.
Worker обслуживает отложенные остановки и победителей без новых poll tick:
раз в секунду добавляет служебную запись в FIFO, которая не обгоняет более
ранние события и не голодает при постоянном потоке новых котировок. `stop_winline_notification_worker`
ждёт поток ограниченное время; test/helper reset отказывается очищать состояние,
пока старая отправка не завершилась, и второй отправитель не запускается.

Шедулер ждёт до ближайшего `_next_poll_mono`, проверяя lifecycle не реже своего
`interval_s` (по умолчанию 1 с). Создание и ускорение поллера будят ожидание;
после долгого опроса нет обязательной дополнительной секунды сна. Настроенный
такт опроса не меняется. Evidence обновляется для попыток, терминалов и ошибок;
`not_due`/`in_flight` сохраняют предыдущий файл. Общий `latest` публикуется один
раз за проход, после файлов отдельных карт.

**Исторический DOM Winline (13.09.2026).** `base/winline_dom_history.py`
включается `WINLINE_DOM_HISTORY_ENABLED=1` (по умолчанию выключен).
Поллер передаёт в `attempt_observer(attempt, result)` фактический вход финального
разбора и метаданные этой попытки. Быстрый путь сохраняет HTML из того же RPC;
полный parser с `capture_dom=True` сохраняет HTML/body/visible из своего чтения,
без дополнительного обращения к странице. В архиве есть время захвата, Python
идентификатор объекта страницы (не PID браузера), ordered canonical key,
номер попытки, producer generation и нормализованный результат с `odds_bettable`.
Для полного пути HTML и body могут быть прочитаны последовательно: архив
сохраняет именно эти входы, а не утверждает атомарность исходного чтения.

На изменении цены/статуса/доступности кнопок/ошибки снимок ставится в очередь
сразу; при одинаковом результате — раз в `WINLINE_DOM_HISTORY_INTERVAL_S`
(15 с, 0 означает каждую попытку). Перезагрузки также сохраняются.
Очередь жёстко ограничена 8 снимками; при перегрузке запись пропускается,
опрос не ждёт. Один поток сохраняет метаданные попытки в неизменяемые
`*.json.gz` (schema 2), а HTML — в `blobs/<sha256>.html.gz`: одинаковый HTML
не дублируется. `index.jsonl` хранит SHA256 и ключ попытки, `status.json` —
состояние/ошибки/пропуски. `read_record(root, index_entry)` проверяет хеши
записи, сжатого blob и HTML, восстанавливает `inputs.html`; читает также schema 1.
Старые записи не мигрируют автоматически. Бюджет включает вложенные файлы,
новую запись/blob/index и резерв для status; новый формат не возобновляет
запись в уже заполненный каталог без изменения каталога или лимита оператором.
Каталог: `runtime/artifacts/odds-winline/dom-history`, либо
`WINLINE_DOM_HISTORY_DIR`. Лимит `WINLINE_DOM_HISTORY_MAX_MIB` (512 МиБ) учитывает
существующие файлы. При заполнении архивирование останавливается с
`budget_exhausted=true`; файлы автоматически не удаляются. `writer.lock`
не допускает двух писателей в один каталог. Входы свыше 4.5 млн символов
отбрасываются целиком с учётом в `oversize`, без незаметного усечения.
Поле evidence `dom_history_queued_id` подтверждает только постановку в очередь;
наличие записи проверяется по `index.jsonl` и SHA256 файла. Сама очередь не
переживает остановку процесса: до 8 ожидающих снимков и один выполняющийся
могут потеряться также при `systemctl stop` (SIGTERM не вызывает `atexit`).
Архиватор намеренно не меняет обработку сигналов основного runtime; `close`
дренирует очередь только в пределах заданного ожидания и сообщает незавершённость.
При ошибке записи следующий одинаковый результат не ждёт интервал выборки.
Ошибка браузера без DOM записывается с `parser_path=unavailable`, без выдуманного
времени захвата. Raw DOM не сохраняется в массиве попыток
...
полученными кэфами в continuous-режиме действует дополнительный таймер отсутствия
рынка: 120 секунд до запроса восстановления, независимо от порога числа промахов.
Ожидание следующей неподтверждённой карты и явно закрытый рынок этот таймер не
запускают. Успешный ответ, закрытие рынка, ошибка браузера и перезагрузка сбрасывают
его; действуют прежние cooldown и settle. Это срок запроса, не гарантия появления
кэфов: занятость браузера и cooldown могут отложить выполнение.
...
| `RELOAD_MISSING_MARKET_SECONDS` | `WINLINE_CURRENT_MAP_RELOAD_MISSING_MARKET_S` | 120 | срок отсутствия рынка живой карты до восстановления; `0` отключает этот таймер |
| shared-page cooldown (cyberscore) | — | 60 | дедуп физических перезагрузок общей страницы между матчами |

На serv1 проверен override `WINLINE_CURRENT_MAP_RELOAD_AFTER_MISSES=114` при такте
3.5 секунды: у Dawn Bulls — MOUZ 12.09.2026 он задержал запрос восстановления
на 508 секунд после первого промаха. Новый таймер не зависит от этого override.

Быстрый collector считает подпись по контексту целевой карточки как при наличии
кэфов, так и при пропаже рынка. Соседние события больше не маскируют её неподвижность.
Если карточки нет или используется полный разбор, подпись остаётся общей для
страницы; evidence различает их через `dom_signature_scope=card|page`. Неподтверждённая
карта в перерыве не перезагружает страницу только из-за неподвижного заголовка.

Общий batch-снимок не обслуживает запрос `controlled_reload`; перед физическим
обновлением старый снимок сбрасывается, а после ошибки навигации новый не
кэшируется до валидного результата страницы. Evidence сохраняет
`acquisition_mode_requested`, фактический `acquisition_mode`, `reload_attempted` и
`market_missing_seconds`. Подавленный общим cooldown запрос не увеличивает
`reload_count` и не сбрасывает таймер восстановления. `reload_attempted=true`
обозначает попытку навигации, включая таймаут, а не её успех.
На правильной странице controlled reload ждёт `commit` до 3000 мс, затем
появление event feed до 1500 мс вместо ожидания DOMContentLoaded до 12 с.
Это пределы двух ожиданий, не всей попытки: дорисовка и разбор идут отдельно.
После ошибки до commit парсер не принимает прежний документ и не пытается
добыть из него цену повторным чтением. При readiness timeout возвращается
`partial_load`; пригодность цены по-прежнему доказывает обычный разбор карты.
Исправление неправильного URL сохраняет отдельный ограниченный goto.
...
## Жизненный цикл карты и служебные сообщения (Winline)

Служебные строки о карте уходят отдельным ботом в админ-чат
(`_winline_build_odds_message` в `base/cyberscore_try.py`): `🆕` первый кэф,
`📊` движение, `🔒` рынок закрыт, `🏁` карта завершена, `⏹️` опрос остановлен,
`🏆` победитель карты.

**Строка `🕐` — хронометраж карты, а не серверное время.** `game_time` из снимка
моста SourceTV досчитывается по `timestamp` снимка до момента съёма кэфов
(`_winline_map_clock_label`; предел досчёта `_WINLINE_MAP_CLOCK_EXTRAPOLATE_MAX_S`
= 120 с — снимок мог протухнуть вместе с картой). Хронометраж хранится по ключу
серии вместе с `map_num` (`_winline_map_clocks`), поэтому чужой карте он не
приписывается. Строка перерыва и уход серии из снимка замораживают последнее
значение: досчитывать его значит показать конец карты на минуты позже настоящего.
Когда мост карту не видит (перерыв, карта не подтверждена источником, опрос
ожидаемой карты), в карточке стоит `🕐 —`: соврать временем хуже, чем показать
прочерк.

Пишет хронометраж reconcile, а он идёт из чтения листинга главным циклом — замер
02.09.2026 на проде: 49 чтений листинга на 147 циклов, примерно раз в 90 с. Этого
мало: карта 2 Team Synapse — 4ikibamboni четыре карточки подряд показывала «9:25»
при живой карте. Поэтому в момент отправки снимок дочитывается сам
(`_winline_refresh_map_clocks_from_bridge`): не чаще раза в 5 с, только в режиме
sourcetv, строки старше 300 с отбрасываются — та же граница свежести, что у
главного цикла.

**Строка `💰` — лидер по net worth.** Рядом с хронометражем карточка показывает
команду, которая ведёт по золоту, и разрыв: `💰 <команда> +4 000`. Источник —
`radiant_lead` строки моста (золото Radiant минус Dire, знак подтверждён в
`sourcetv_probe`): при `radiant_lead >= 0` впереди Radiant, иначе Dire. Лидер
называется ПО ФАКТИЧЕСКОЙ СТОРОНЕ строки моста, а не по порядку team1/team2 в
ключе карты — между картами серии команды меняются сторонами, и ключ несёт
прежний порядок имён (`_winline_net_worth_label`). Хранится и обновляется вместе
с хронометражем (те же `_winline_map_clocks` и `_winline_bridge_map_clock`),
поэтому чужой карте не приписывается. Строка добавляется только в карточки живых
кэфов (`🆕`/`📊`/`🔒`/`🏁`) и стоит перед `🕐`; служебные `⏹️`/`🏆` её не
получают. Ничья (`radiant_lead == 0`), отсутствие строки моста или чужой номер
карты, а также плейсхолдер-имя стороны («Radiant»/«Dire») строку подавляют:
показывать «+0» или выдуманного лидера хуже, чем промолчать.

**Потолок опроса живую карту не гасит.** `SAFETY_CEILING_SECONDS` (90 минут)
существует ради опроса, который некому остановить: когда реестр не знает серии,
предикат отвечает «карта текущая» бесконечно. Пока реестр ПОДТВЕРЖДАЕТ именно эту
карту (`_winline_current_map_is_current` возвращает `confirmed=True`), потолок
продлевается шагами до абсолютного предела, а опрос продолжается
(`_extend_safety_ceiling`). Цена ошибки заплачена 31.08.2026: карта 3
Inner Circle x Insanity — 4ikibamboni шла 95 минут, потолок сработал ровно на
90-й (22:59:32) и отправил «опрос остановлен», после чего опрос той же карты
завёлся заново и доложил настоящий конец в 23:04:11.

| Константа | env | Значение | Смысл |
|---|---|---|---|
| `SAFETY_CEILING_SECONDS` | — | 5400 | первый потолок жизни опроса |
| `SAFETY_CEILING_EXTENSION_SECONDS` | `WINLINE_CURRENT_MAP_CEILING_EXTENSION_S` | 1800 | шаг продления, пока карта подтверждена |
| `SAFETY_CEILING_MAX_SECONDS` | `WINLINE_CURRENT_MAP_CEILING_MAX_S` | 14400 | абсолютный предел, после него терминал |

**«Опрос остановлен» откладывается.** Недоказанный терминал (потолок, молчащий
источник) — это «мы не знаем», а не событие: тот же ключ обычно заводится заново
за десятки секунд. Сообщение копится в `pending_stop` и уходит только если за
`WINLINE_ODDS_STOP_NOTICE_HOLD_S` (180 с) по ключу не появилось ни нового опроса,
ни доказанного конца карты (`_winline_flush_pending_stop_notices`, зовётся каждым
тактом шедулера). Доказанный конец карты отложенное объявление снимает.

Дедупликации против предыдущего `stopped` во `_winline_flush_pending_stop_notices`
нет: объявление уходит по разу на КАЖДЫЙ недоказанный терминал. Значит источник
недоказанных терминалов обязан заканчиваться — см. «Ожидание следующей карты —
одноразовое» ниже.

**Победитель карты — цепочка источников.** В `🏁` добавляется строка
`🏆 победа: <команда>`; имя ищет `_winline_resolve_map_winner` в порядке
«дешевле и независимее сначала»:

1. **Счёт серии из снапшота моста** (`_winline_series_wins_seen` +
   `_winline_series_wins_baseline`) — ноль сети. `radiant_series_wins` /
   `dire_series_wins` пишет `sourcetv_probe` (Steam WebAPI первично, GC-протобуф
   запасной). Счёт держится ПО КОМАНДАМ, а не по сторонам: между картами команды
   меняются сторонами. Победитель называется только при полном доказательстве —
   счёт сходится с номером карты (`sum == N`), ровно одна команда прибавила одну
   победу, вторая не двинулась (`_winline_winner_from_wins_delta`); базовым
   считается только счёт, согласованный с номером (`sum == N - 1`), он
   сбрасывается на первой карте новой серии той же пары и протухает за
   `_WINLINE_SERIES_WINS_BASELINE_TTL_S` (12 ч). Карта 1 читается и без базового:
   1:0 после неё однозначно.
2. **Steam WebAPI `GetLiveLeagueGames?league_id=…`** — один лёгкий HTTP (4 с,
   `keys.STEAM_API_KEY`, тот же вызов, что в `base/gc_bot.py`), лига берётся из
   ключа серии. Покрывает случай, когда серии уже нет в снапшоте моста, но она
   ещё в живых списках Steam. `GetMatchDetails` для этого мёртв: HTTP 500 и на
   свежем, и на старом матче.
3. **Кэш Stratz** (`_winline_winner_from_stratz` → `stratz_map_result.series_history`)
   — чтение с диска БЕЗ сети: `query` подменяется заглушкой, возвращающей None, и
   `team_matches` отдаёт даже протухший кэш, не выдумывая. Кэш греет фоновый
   прогрев живого ELO (опрос карты раз в полминуты начиная примерно через
   четверть часа после её конца). Это единственный источник, знающий исход
   ПОСЛЕДНЕЙ карты серии: следующего лобби нет, поэтому ни мост, ни Steam счёт
   серии уже не покажут. Id сравниваются в обоих пространствах (наш и
   `resolve_team_id`), чужой id — это None, а не догадка. Сетевой поход Stratz
   здесь запрещён намеренно: он идёт через пул пар ключ↔прокси, где мёртвая пара
   стоит ровно таймаут, и висел бы в потоке-шедулере, который снимает линию.
4. **OpenDota по `match_id`** (`_winline_fetch_map_winner` → `radiant_win`; тот
   же источник закрывает серии в живом ELO). Спрашивает про матч напрямую и
   потому надёжнее всех, но он один и он отказывает.

Первые два источника работают и без `match_id` — то есть и для опроса,
заведённого в перерыве; Stratz и OpenDota без id матча молчат.

Браузерных источников (cyberscore/dltv) в цепочке нет намеренно: браузер один и
общий с поллером кэфов, такой запрос отнял бы время съёма линии у живых карт.

Факт конца карты именем не задерживается: если ни один источник не ответил,
`🏁` уходит без строки, а имя догоняет отдельным сообщением `🏆`
(`_winline_flush_pending_map_winners`). Замер аварии api.opendota.com 03.09.2026
(522 и HTTP=000 с двух независимых сетей при живых winline.ru и example.com):
карта 1 PuckChamp — Team Spirit Academy ушла без победителя, хотя Steam в тот же
момент отдавал по лиге 19944 `wins 0:1`; карта 1 Pipsqueak + 4 — DYNASTY
получила имя через 600 с — ровно когда появилось лобби следующей карты; а карта 3
той же серии (последняя, `8980994942`, терминал 21:15:47) не получила имени и
через полтора часа — лобби дальше не будет, Stratz карту ещё не опубликовал,
OpenDota лежал.

| env | Значение | Смысл |
|---|---|---|
| `WINLINE_ODDS_STOP_NOTICE_HOLD_S` | 180 | ожидание перед объявлением остановки опроса |
| `WINLINE_NEXT_MAP_WAIT_S` | 2700 | окно ожидания следующей карты; отрабатывает один раз |
...
| `WINLINE_MAP_WINNER_STEAM_ENABLED` | вкл | запасной источник — Steam WebAPI; в тестах выключен |
| `WINLINE_MAP_WINNER_STRATZ_ENABLED` | вкл | запасной источник — кэш Stratz (без сети); в тестах выключен |
...
тикают два водителя: поток-шедулер (основной такт) и backup-тик главного цикла
после `general()` (`from_main_loop=True`). Backup-тик в чат не пишет намеренно —
блокирующая отправка не должна удлинять цикл ставок, — но терминал поллер отдаёт
РОВНО ОДИН раз, и снятый backup-тиком он исчезал навсегда: поллер мёртв, нового
повода сообщить не будет. 02.09.2026 Team Synapse — 4ikibamboni: карта 1
терминалилась в 19:55:24 (`map_end_proven=True`, `map_confirmed_live=True`,
evidence записан), «🏁 карта завершена» не ушло, а в 19:55:25 чат увидел только
«🆕 Winline · карта 2». Теперь такой терминал кладётся в
`_winline_deferred_terminals` и дообъявляется следующим тактом шедулера
(`_winline_flush_deferred_terminal_notices`, раньше отложенных остановок:
доказанный конец карты снимает `pending_stop` того же ключа). Отказ Telegram
(`delivered=false`) больше не теряет сообщение: объявление повторяется до
`_WINLINE_DEFERRED_TERMINAL_RETRIES` раз, но не повторяется, если конец карты уже
объявлен (дедуп по `kind == "terminal"` в состоянии чата).

**Журнал отправок.** Каждая карточка и каждый отказ записываются в
`WINLINE_ODDS_TELEGRAM_SENT_PATH` (`wall`, `kind`, `canonical_key`, `delivered`,
`message`). До 02.09.2026 отправленные сообщения не записывались нигде, и оба
разбора того дня упирались в невозможность сказать, что именно ушло в чат.

Запрос победителя идёт из потока-шедулера, который ведёт опрос ВСЕХ живых карт,
поэтому таймаут HTTP короткий (4 с) и за один такт делается не больше одного
обращения: иначе пачка доигранных карт при молчащем источнике складывала бы
таймауты в один такт и останавливала съём линии на живых картах.

**О карте, которой не было, не сообщают ничего.** Опрос следующей карты
заводится ЗАРАНЕЕ: рынок карты 2 Winline открывает в перерыве, когда у Valve ещё
жива доигранная карта 1, и `_winline_sourcetv_map_num` в этот момент сдвигает
номер карты на единицу (признак вынесен в `_winline_sourcetv_postgame_intermission`:
статус live, драфт явно стёрт, game_time ≥ 10 мин, есть убийства). Всё, что взято
из ТАКОЙ строки моста, принадлежит ПРЕДЫДУЩЕЙ карте, поэтому:

- запись реестра от строки перерыва помечается `bridge_confirmed=False`
  (`_reconcile_winline_sourcetv_polling`), и её исчезновение НЕ доказывает
  конец нашей карты;
- сдвиг номера делается ТОЛЬКО когда счёт серии ещё не засчитал доигранную карту
  и есть с чем сравнивать (`game_number > 0 and played < game_number`): при уже
  обновлённом счёте `played + 1` — это и есть следующая карта, а второй сдвиг
  перескакивал бы через неё (Bo5 при 1:0 давал карту 3 вместо 2; без номера у GC
  при счёте 1:0 — тоже 3 вместо 2);
- `match_id` с такой строки в опрос не уезжает (`_winline_card_match_id` → None),
  а приходит от моста вместе с подтверждением карты
  (`_winline_current_map_is_current` → `match_id`, поллер принимает его в
  `_absorb_confirmation`);
- терминал несёт `map_confirmed_live`; если источник карту ни разу не подтвердил,
  доказанный конец ПОНИЖАЕТСЯ до недоказанного: «карта завершена» и победитель
  не объявляются никогда, остаётся отложенное «опрос остановлен», которое само
  снимется, если опрос той же карты возобновится в окне ожидания. Молчать
  совсем нельзя: чат уже видел «🆕» с ценами по этому ключу, а карта могла и
  сыграть — 22.08.2026 карта `8959362208` не появилась у Valve вовсе;
- реестр, показывающий карту РАНЬШЕ нашей, — это перерыв (`awaiting_map_start`,
  опрос продолжается), а не смена карты; `map_rollover` остаётся только для
  обратного хода, когда мост показывает карту ПОЗЖЕ нашей.

Победитель дополнительно проверяется дважды: один матч Valve закрепляется ровно за
одной картой серии (`_winline_claim_winner_match`), и матч, доигранный ДО начала
опроса карты, отвергается по `start_time + duration` из ответа OpenDota
(`map_started_at`). Цена ошибки заплачена 01.09.2026: серия
PuckChamp — Klim Sani4, «🏁 Winline · карта 2 · 🏆 победа: Klim Sani4» в 13:07:33
по карте, которая не начиналась, — с победителем карты 1 (матч 8977252978,
доигран в 12:43:31).

**Идентичность серии приводится к именам команд.** Ключ серии строится из
`league_id` и пары team id моста (`_winline_sourcetv_series_key`), а мост даёт
одной команде РАЗНЫЕ id на разных картах одной серии: 02.09.2026 Yangon
Galacticos шёл под `7653080` на карте 1 и под `8944230` на карте 2 серии с
InterActive Philippines. Серия расщепилась на два ключа, и карту 2 одновременно
опрашивали два поллера — настоящий (такт 3.5 с, `match_id` 8978758321, цены
1.85 → 5.50) и сирота из ожидания следующей карты (такт 60 с, `match_id=None`).
Оба читали одну карточку Winline, в `runtime/winline_odds_history.jsonl` лежат
зеркальные цены одного момента (11:42:55 → 2.90/1.32 и 11:43:26 → 1.32/2.90), а
сирота без подтверждения моста давал только недоказанные терминалы.

Теперь id канонизируется по имени (`_winline_canonical_team_id`): id из моста,
уже известный справочнику tier1/2 под этим именем, остаётся собой (ключи в логах
и истории читаемы), а неизвестный сводится к минимальному известному — приоритет
тот же, что в `_resolve_known_team_id_without_side_effects`. Решение запоминается
на имя ОДНОКРАТНО (`_winline_team_id_canonical`): справочник растёт (overlay
динамического onboarding'а), и смена выбора посреди серии расщепила бы ключ точно
так же, как флип id у моста. Индекс имени собирается раз в 10 минут
(`_WINLINE_TEAM_ID_INDEX_TTL_S`). Имена-плейсхолдеры («Dire», «Radiant»,
`ANONYMOUS_TEAM_NAMES`) не сводятся: под ними ходят разные команды. Команды,
которых нет в tier1/2, остаются на id из моста — их флип лечит только одноразовое
ожидание ниже. `league_id` в ключе остаётся: та же пара Kinetix + Yangon
встречалась под лигами 18865 и 19944, и склейка по паре без лиги смешала бы две
разные серии.

**Ожидание следующей карты — одноразовое.** Окно `WINLINE_NEXT_MAP_WAIT_S`
(45 мин) отрабатывает один раз: если карта так и не появилась в мосте, запись
ожидания помечается исчерпанной (`exhausted_at`) и больше не взводится, пока
серия снова не появится в снимке. Запись реестра умершей серии не удаляется
никогда, а `_winline_next_map_certain` по замороженному счёту каждый проход
отвечает «следующая карта неизбежна», поэтому прежнее удаление записи означало
бесконечный цикл «45 минут опроса карты, которой нет → недоказанный терминал →
`⏹️ опрос остановлен` в чат». Цена ошибки заплачена 02.09.2026: по серии Yangon
Galacticos — InterActive Philippines карта 2 доиграна в 11:50, а сообщение
приходило каждые ~48 минут до рестарта в 17:48 — 8 повторов, последний в 17:25:35
(= 16:37:34 + 45 мин окна + 180 с hold); второй зомби (Yangon — Team Kinetix)
сделал 14 циклов с 07:03 без единого кэфа и ~630 холостых заходов в браузер на
ОБЩЕЙ странице, то есть воровал время съёма у живых карт. Всего за день 38
перезапусков поллеров на 16 ключей. Карта, начавшаяся позже окна, приходит в мост
и заводится обычным путём — от живой записи, а не от замороженной.
...
случая — `series_last_map=True` и у рынка карты нет доступной пары кэфов.
Открытый рынок карты имеет приоритет. С 08.09.2026 пустая/заблокированная
строка карты не мешает взять открытый двухисходный «Матч»: на доказанной
решающей карте это тот же исход. Конфликтующие цены рынка карты отклоняются.
...
- обе кнопки именно рынка «Матч» должны быть доступны; блокировка отдельного
  рынка карты не переносится на принятый рынок «Матч»;
- порядок сторон выравнивается по тексту карточки (`_winline_team_order`).

**Вычисление «последней карты».** `cyberscore_try._winline_series_last_map(map_num,
team1, team2, *payloads)` читает свежий срез SourceTV (`runtime/sourcetv_matches.json`,
кэш по mtime в `_winline_series_rows_cache`; принимает и dict, и list) либо уже
прочитанный payload матча — payload главнее файла. По `series_type` (0=BO1, 1=BO3,
2=BO5, 3=BO2): Bo3 решающая при счёте 1:1 и `map_num == 3`, Bo5 — при 2:2 и карте 5,
Bo2 — всегда карта 2; формат неизвестен, счёт нечитаем или матча нет в срезе —
`False`.

**Проводка флага (с 05.08.2026).** Раньше флаг не передавал никто, и промоция не
срабатывала ни разу (05.08.2026 `No Hoodwink — Lynx`, Bo3 1:1, карта 3: в карточке
только `Матч 2.55 1.42`, поллер отдавал `market missing`). Теперь
`ensure_winline_current_map_polling(series_last_map=...)` кладёт флаг в реестр серий,
а `_winline_registry_series_last_map(series)` отдаёт его обоим путям разбора:
быстрому (`_winline_fast_collect_from_payload` → `_extract_winline_current_map_winner`)
и полному (`parse_site_in_camoufox_page(series_last_map=...)`, где DOM-разбор
решающей карты проверяет доступность рынка до подстановки «Матч»;
разбор остальных карт сохранён). В evidence
поллера появились `series_last_map` и `odds_promoted_from_match`.

**Провенанс стороны в evidence (с 05.08.2026).** `p1_odds/p2_odds` уже приведены к
порядку ЗАПРОСА, поэтому по одному снимку нельзя было доказать, что цена не уехала
на другую команду — приходилось сверять с параллельным парсером. Теперь
`_WinlineMapExtract`/`SiteResult` несут `card_team_order` (наши имена из SourceTV в
том порядке, в каком их написал Winline) и `card_odds` (пара цен БЕЗ разворота), а
поллер добавляет `dom_age_seconds` — сколько секунд не менялась подпись DOM к моменту
опроса. Вместе они отличают «сторона уехала» от «цена отстала на замершей странице».
...
**Тесты:** `base/tests/test_winline_last_map_match_promotion.py`,
фикстуры `winline_bo2_match_three_way_20260802.html` (Bo2 — запрет) и
`winline_last_map_match_only_20260802.html` (Bo3 — промоция).
Регрессия 08.09.2026 мутирует эти сохранённые карточки: пустой/заблокированный
рынок карты + открытый «Матч», включая выход быстрого и полного коллекторов.
Это воспроизведение класса дефекта; HTML ранних минут Power Rangers — Huligani
не сохранился, поэтому точная историческая причина задержки не доказана.
...
Ночной шаг 8 в `scripts/run/rebuild_prematch_snapshot.sh`: stop → rebase полного состояния → `convert_state_to_delta.py --if-stale` → sidecar массивов → очистка map_id_check → start. Пропуск обновления дельты после смены снимка возвращает live-путь к тяжёлому полному состоянию; ошибка подготовки выводится как предупреждение и не блокирует запуск сервиса. `--if-stale` сравнивает у существующей дельты только `base_reference_timestamp`/`base_model_config_signature` со снимком — если они совпадают, конвертер выходит как «дельта уже совпадает со снимком» и НЕ пересобирает дельту, даже когда состояние только что было принудительно перебазировано `rebase_runtime_model_state.py --force` без смены снимка (E-255: рантайм при этом читает старую дельту). После форсированной перебазировки с неизменным снимком запускать конвертер без `--if-stale`, при остановленном сервисе.
...
| `base/sourcetv_probe.py` | Steam GC/SourceTV producer: находит live-матчи/составы/драфт и пишет bridge JSON, который читает `cyberscore_try.py` при `DLTV_SOURCE_MODE=sourcetv`; CLI `--username`, `--password`, `--league`, `--match`, `--interval`, `--login-only`. **Первый гейт allowlist'а лиг** — отброшенный здесь матч не доезжает до моста вообще, поэтому условный допуск зеркалит консумера: `_league_admission`, `_league_tier_gated_admission`, `_note_rejected_league` (см. раздел `base/league_keywords.py`). На serv1 это systemd-юнит `sourcetv-probe.service` + watchdog `sourcetv-probe-watchdog.timer`, перезапуск только через systemctl. Позиции сторон — `_resolve_positions`: override → мода истории про-матчей (команда ≥3, иначе все команды) → при конфликте/пропусках перебор 5! перестановок по `hero_position_stats.json` + 1.0 за совпадение с историей + `DLTV_ROLE_WEIGHT` (0.2) за совпадение с ролью игрока на DLTV (E-333); ветка raw (полная непротиворечивая история) DLTV не запрашивает. Журнал `runtime/pos_resolution.jsonl`: поля `dltv_known`/`dltv_matched`, у слота — `dltv`. |
| `base/dltv_player_roles.py` | Роль игрока (1..5) с DLTV для перебора позиций пробы: `lookup_roles(account_ids, *, deadline_s=None, fetch=None, now=None) -> {account_id: role}` (никогда не бросает), `parse_search_role(payload, account_id)`. Источник `GET https://dltv.org/api/v1/search?q=<account_id>` (открытый JSON; берётся только игрок с `steam_id == account_id`). Кэш `runtime/dltv_player_roles_cache.json` (`DLTV_ROLE_CACHE_PATH`), TTL: найдено 24 ч, нет игрока/роли 12 ч, ошибка сети 10 мин; общий дедлайн вызова `DLTV_ROLE_LOOKUP_DEADLINE_S` (6 с); `DLTV_ROLE_LOOKUP=0` — откат без сети. |
...
- `get_pros_playback(max_age_days=85, out_dir=None, limit=None)` — сбор СОБЫТИЙ внутри карты: `deathEvents` (кто кого убил, чем), `purchaseEvents` (во сколько куплен предмет), `playerUpdateLevelEvents` (во сколько взят уровень). Один матч = один запрос: батч закрыт (`matches(ids:)` — «User is not an admin»), а playback внутри `teams -> matches` не влезает в бюджет сложности 300 000.
- `get_playback_new(ids, out_dir, ..., query=None)` — сам сборщик; `query` выбирает форму запроса. Формы: `PLAYBACK_QUERY` (первый проход) и `PLAYBACK_QUERY_V2` (добавлены `assist` — список помогавших, `timeDead`, координаты убийства). Новые поля дописываются В КОНЕЦ записи смерти, поэтому читатели старого формата (`ev[0..6]`) работают на смешанном корпусе.
- Ограничения Stratz, проверенные 15.08.2026 по заголовкам ответа:
  ...
  - квота **своя у каждого ключа**: `x-ratelimit-limit-day = 15000` у каждого, при этом `retry-after` у ключей различается (39852/39851/39851/39850 c) — у общего счётчика момент сброса был бы один. Ранняя запись «квота общая на аккаунт» была ошибкой: счётчики совпадали потому, что сборщик раскладывал нагрузку поровну;
...
- **Pub-пагинация (19.09.2026).** `iter_pub_pages(player_ids, *, skip=0, batch_size=5, batch_concurrency=1, start_date_time=None, player_ids_check=False, existing_match_ids=None)` — async-поток `(completed_ids, new_matches, new_player_ids, failed_ids)`. Он обходит глобальные BFS-очереди одинакового `skip`: HTTP-запрос содержит максимум 5 игроков, а хвост `skip=N` отправляется только после исчерпания меньшего курсора, поэтому продолжившие игроки успевают заполнить пачку. Полная страница идёт дальше, если все валидные даты в окне, даже если все ID уже известны; упорядоченная страница с датой до cutoff завершает игрока. `null`/некорректный/неупорядоченный timestamp включает sticky conservative traversal; короткая страница завершает игрока. На неполном обходе за `PUB_PAGINATION_MAX_SKIP=100000` выбрасывается ошибка, не тихий обрыв. GraphQL `errors`, missing/partial player response — ошибка контракта; транспортный сбой после трёх попыток возвращает `failed_ids` без отметки этих игроков. `get_maps_new()` потребляет поток постранично, сбрасывает pub-данные также каждые 25 страниц без terminal ID и не обновляет `last_crawl_completed_utc`, если были failed pages. Чекпоинт хранит уже записанные матчи и terminal players, но не миграцию per-player BFS skip: после рестарта незавершённый игрок безопасно повторяет `skip=0` через дедуп. Дедуп `existing_match_ids` публикуется только после успешного разбора одной bounded page-task, без `await`; pro-ветка не использует этот курсор. С 25.09.2026 ошибка контракта после ретраев не роняет обход: страница уходит в `failed_ids`/счётчик `skipped`, чекпоинт и итог печатают `skipped N`, merge собранного идёт ДО `RuntimeError("pub crawl incomplete")`; фатальным остался только `PubPaginationCapError`. Спасение несобранных temp-файлов без нового обхода: `python3 base/maps_research.py --merge-temp-files [--dry-run]` (`salvage_pub_temp_files`, тот же streaming-merge и дедуп; temp-файлы не удаляются; `--dry-run` только считает и ничего не пишет — ни каталогов, ни part-файлов, ни processed_ids.txt/summary/manifest/counters; битые temp-файлы дают `MergeIncompleteError` и non-zero exit, хорошие при этом сливаются; `cleanup=True` при битых файлах каталог temp не удаляет). `save_temp_file` пишет temp атомарно (`<имя>.txt.tmp` + fsync + `os.replace`; сканы temp берут только `*.txt`), поэтому убитый посреди записи обход не оставляет обрезанный файл, который валил бы каждый следующий merge. Salvage на serv1 запускать только при остановленном обходе: `flock -n runtime/pub_recrawl.lock venv/bin/python3 base/maps_research.py --merge-temp-files` (сначала `--dry-run`). `build_laning_corpus.SKIP_FILENAMES` пропускает `part_counters.json` наравне с summary/manifest.
...
- **Регулярный pub-сбор (19.09.2026).** `bash scripts/run/get_pubs_full_recrawl.sh --since EPOCH` запускает `base/pub_recrawl.py` через nohup; `--since` обязателен только при первом запуске, затем вызывается без аргументов. Раннер использует существующий `STRATZ_PROXY_MAP`, принудительно `PUBS_IDS_MODE=file`, запрещает ограниченный `PUBS_MAX_PLAYERS`. `runtime/pub_recrawl.lock` исключает одновременные запуски; `runtime/pub_recrawl.json` хранит cutoff, начало обхода и статус running/failed/complete. Через пять суток после начала успешного обхода разрешён следующий; его cutoff — начало предыдущего минус сутки, чтобы не пропускать матчи за время долгого обхода. Перед новым обходом курсор игроков переименовывается в уникальный backup; дедуп матчей сохраняется. После сбоя продолжается тот же cutoff и графовый курсор; незавершённые игроки повторяются с skip=0. Старый несовместимый checkpoint `analise_pub_matches/maps_state.json`, если существует, блокирует запуск до проверки. PID: `runtime/get_pubs.pid`; логи: `runtime/artifacts/pubs-rebuild/get_pubs_*.log`. Расписание задаёт внешняя автоматизация, сам раннер проверяет срок и блокировку.

### `opendota_research.py` — сбор разобранных матчей из OpenDota

Второй источник событий карты, по покрытию и плотности лучше Stratz-playback.

- `collect_pro_patch(ids_file, out_dir, include_direct=False, pace=1.05, loop_sleep=3600)` — проход за проходом, пока очередь не кончится; возобновляемость по id, реально лежащим в `*.jsonl.gz`.
- `collect(ids, out_dir, endpoints, ...)` — по одному потоку на адрес; адрес, выбравший суточную квоту, выходит сам.
- `proxy_endpoints(include_direct=False)` — список адресов: файл из `OD_PROXIES_FILE` (по строке на адрес) либо `keys.api_to_proxy`. Свой IP по умолчанию НЕ занимаем: с него в OpenDota ходит прод (`cyberscore_try.py`).
- `compact(d, npc2id)` — ужимает ответ с 380 КБ до 13 КБ (3.5 КБ в gzip): из `purchase_log` остаётся первая покупка каждого предмета, поминутные ряды и чат выбрасываются.
- Что даёт (замеры 15.08.2026):
  - разобрано **29 из 30** карт нашего корпуса против 62% у Stratz, включая **9 из 10** тех, по которым Stratz playback не отдаёт; разбор хранится без срока, поэтому по 7.41 доступно **42 560** карт вместо 23 887;
...
`base/tools/build_fresh_phase_holdout.py` фиксирует новые pro ID после старого public/pro cutoff и исключает пересечения. `base/tools/tune_draft_phase_models.py` выбирает C/design по embargoed validation, сохраняет хеши исходников/корпуса/артефактов в `status.json`; незавершённый output и frozen-forward corpus отклоняются. `base/tools/evaluate_phase_forward.py` принимает четыре DONE-кандидата, сверяет report/model SHA и baseline с production audit до чтения holdout, затем создаёт неизменяемый `forward_started.json` и однократно пишет `forward_results.json`. CLI и ограничения оценки: [E-278](experiments/E-278-phase-regularization-forward.md).
...
`_ml_dispatch_deliver_decision` передаёт в общий delivery gate текущую карту
и observation, привязанный к точному `match_key`. Timestamp/status источника
сохраняются: устаревший или завершённый матч отклоняется существующим валидатором.
Это wiring общего gate; пороги ML/Winline и включение odds-режима не меняет.
...
| `PREMATCH_ML_ENABLED` | `0` | с 24.09.2026 (решение владельца: 🤖 модель не лучше ELO — E-307 live 69,0 % против 70,8 %, E-308/E-309 без значимого перевеса). `0`: общая 35-признаковая prematch-модель не загружается (~4 ГБ), строки «🤖 ML-модель» нет, голоса `prematch` у диспетчера нет, отдельной ставки `prematch_model_bet` нет, `BET_REQUIRE_WIN_MODEL`/`blocks_veto`/`draft_veto`/`model_bet` не действуют. Early NW/Early Win/Late идут через `laning_serving.fallback_verdicts`, панель w_* и kills30 — через `win_model_veto._off_auxiliary_panels` (prod35 заполняется нейтрально, журнал `ml_panel.jsonl` не пишется). Сторож позиций E-296 сохранён: `prematch_scorer.light_position_hard_slots` грузит из артефакта только `accounts[:, (0, 2)]` и `acc_pos`. `1` — прежнее поведение целиком |
| `ML_DISPATCH_WIN_MODELS` | `late,all,early_win,early_nw` (+`prematch` только при `PREMATCH_ML_ENABLED=1`) | какие модели считаются "поддержкой" win-маркета (все четыре с 12.09.2026 16:50 — правило владельца «хоть одна ★ → сигнал»; до этого `early_nw` в дефолте не было; 🤖 Prematch добавлен 15.09.2026 — офлайн сильнейшая модель, ★≥0.60 даёт 71.2% n=9389, её собственный путь `prematch_model_bet` заблокирован `_dispatch_mode_reject_for_delivery` с 12.09.2026, это wiring возвращает её мнение в ставку; НЕ в `EARLY_ONLY_BLOCK_MODELS`, откат — `ML_DISPATCH_WIN_MODELS=late,all,early_win,early_nw`); Late/All всегда вето-модели независимо от этого списка |
...
| `ML_DISPATCH_MIN_ODDS_MARGIN` | `0.12` | запас в `min_odds = round(1/(expected_wr-margin), 2)`; с 24.09.2026 0,12 (решение владельца, E-327 часть 5: `expected_wr` переуверен на 12 п.п.); `=0` возвращает прежний нулевой пол. WIN-сообщение печатает «Ставить от кэфа X.XX» второй строкой. При `--no-odds` пол сверяется со свежей ценой in-process опроса Winline той же карты и команды (`_ml_dispatch_fresh_winline_price`: ≤300 с в игре, ≤1800 с до горна; закрытый рынок, старая или неоднозначная котировка цены не дают → сигнал уходит с полом в тексте); цена ниже пола → отказ `ml_min_odds_below_floor`, `price_source=winline_poll` |
