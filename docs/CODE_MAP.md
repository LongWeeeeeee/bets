...
- запись реестра от строки перерыва помечается `bridge_confirmed=False`
  ...
  конец нашей карты;
- сдвиг номера делается ТОЛЬКО пока счёт серии не засчитал доигранную карту
  (`played < game_number`): при уже обновлённом счёте `played + 1` — это и есть
  следующая карта, а второй сдвиг перескакивал бы через неё (Bo5 при 1:0 давал
  карту 3 вместо 2);
...
- терминал несёт `map_confirmed_live`; если источник карту ни разу не подтвердил,
  доставка молчит совсем — ни `🏁`, ни отложенного `⏹️`;
