...
- Записей на матч: early-тип `295 → 175` (solo 10 + 1v2 50 + 2v1 50 + 1v1 25 + with 20 + trio 20), kills_window `275 → 55`.
...
- `KILLS_WINDOW_BUILD_HIGH_ORDER` (default `0`) — слои 1v2/2v1 в kills_window НЕ пишутся: политика `core_1v1_with` их не читает (fallback включается только когда 1v1 и with оба пусты, покрытие обоих на 1099 про-картах 1099/1099). Ребилд с `=1` возвращает их — нужно для политик `first_hit`/`blend_all`/`best_abs` и слоевых A/B.
