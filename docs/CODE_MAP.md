...
| `_deliver_and_persist_signal(...)` | 27788 | единая точка доставки; half-stake ELO, согласие предматчевой ML и `late_model_against` (любая обычная ставка, если Late ML против target) — до prepare/dedup. Отказ любого из этих гейтов пишет `kind=reject` + `bet_message` в журнал вердиктов (хвост `tail_log`), матч не закрывает |
...
| `BET_REQUIRE_LATE_WIN_MODEL` | `1` | `0` снимает гейт late-модели (`_late_win_model_reject_for_delivery`). `late_model_against` — любая обычная ставка, если строка `🕑 Late ML-модель` называет не target. `late_model_missing` — только late-driven: без строки late-ставки нет |
