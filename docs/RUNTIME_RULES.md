...
- `TEAM_KILLS25_SHADOW_ENABLED=0` — включает скоринг/JSONL-аудит schema `team_kills27_shadow.v1` для кандидатов NW60 с `hits>=2`; основной dispatch и stake не меняются.
- `TEAM_KILLS25_SHADOW_MODEL_PATH=ml-models/team_kills27/team_kills27_shadow.json` — frozen JSON logistic artifact на 93 признака (90 draft/ELO + три latest-patch roster feature).
...
- `TEAM_KILLS25_TELEGRAM_MIN_PROBABILITY` по умолчанию берётся из artifact (`1 / 1.8 = 0.5556`, выбран на old chronological validation); `TEAM_KILLS25_TELEGRAM_MIN_WR=60` задаёт дополнительный нижний WR-gate.
...
- Последние 30 карт с overlap состава `>=4` дают три latest-patch model features: games, mean kills и rate `kills>=27`. Hard gates по min games/mean kills отсутствуют; sender fail-closed только при недоступном источнике, а пустой валидный sample обрабатывается model imputer.
