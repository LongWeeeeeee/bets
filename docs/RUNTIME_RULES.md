...
- `TEAM_KILLS25_TELEGRAM_ENABLED=0` — отдельная отправка ML-qualified рекомендаций сейчас оставлена выключенной; при последующем включении требует `TEAM_KILLS25_TELEGRAM_BOT_TOKEN` и `TEAM_KILLS25_TELEGRAM_CHAT_ID` из закрытого service env-файла.
