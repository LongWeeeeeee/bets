...
Winline-first admission (E-268, 09.09.2026): порядок «сначала матч SourceTV, потом поиск в Winline» ронял матчи с неполным опознанием до опроса букмекера. Теперь раз в цикл снимается общий live-снимок ленты Winline (фоновая нить, цикл его не ждёт), и пара SourceTV, найденная в живой карточке, допускается мимо league-allowlist / team_id-гейта / league-denylist — неизвестная сторона остаётся неизвестной (id 0, tier 3 по правилам tier 2, без онбординга). Нет снимка или совпадения — прежний путь (fail-open, kill-switch `WINLINE_FIRST_ENABLED=0`).
