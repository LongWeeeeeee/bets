...
Тот же снапшот содержит `team_kills_history_by_team_id`: для каждого Dota team ID до 100 последних карт с `match_id`, timestamp, пятью player IDs и числом убийств команды. Kill arrays из raw Stratz суммируются загрузчиком; карты без валидных kills/пятёрки игроков пропускаются, архивные дубликаты удаляются по `match_id`. JSON заменяется атомарно, а legacy-снапшот без kills-history пересобирается при обычном `rebuild_if_missing=True`.

### Team kills≥25 shadow (`base/team_kills25_shadow.py`)

После расчёта полного draft payload и pre-map ELO кандидат `early_output` WR60 с минимум двумя STAR-hit записывается в JSONL shadow. В record входят `nw_hit_count`, максимальный подтверждённый `nw_max_wr`, все шесть метрик каждого блока (`early_nw`, `early_win`, `late`, `all`) отдельно, их абсолютные величины/agreement и target-aligned ELO. Frozen JSON logistic artifact добавляет `ml_probability`.

Live runtime передаёт account IDs обоих текущих составов. Для target team kills25-контур берёт из ELO-снапшота только более ранние карты того же `team_id` с overlap текущего состава минимум 4/5, повторно исключает дубликаты `match_id` и ограничивает окно последними 30 подходящими картами. Текущая карта исключается по `match_id`, а data leak от будущих/одновременных карт — по `timestamp >= observed_at`. В `roster_kills` фиксируются sample size, mean/median kills и hits/rate для `kills >= 25`.

Опциональный изолированный sender отправляет рекомендацию через отдельный Bot API token только после двух групп условий: прежний ML/WR gate и roster gate `matches >= 6`, `mean_kills >= 23` (defaults; пороги настраиваются env). Недоступная история, меньше четырёх current account IDs или малый sample блокируют только этот sender. Telegram-сообщение показывает roster sample, среднее, медиану и долю 25+. Антидубликат нормализует меняющиеся DLTV poll/score keys до стабильного Dota `match_id` и до HTTP-вызова атомарно фиксирует persistent at-most-once claim; legacy sent-log также нормализуется. Shadow-аудит и основной STAR/stake/bookmaker dispatch от roster gate не зависят.
