...
Tombstone, доказанный правилом (2) team-pair, хранит фактически использованный ключ снимка (`snapshot_covered_match_id`). Следующий ребейз пытается подтвердить его тем же ключом в новом снимке; если не находит — tombstone не отбрасывается как ошибка, а остаётся до истечения окна `SNAPSHOT_RECENT_RESULT_COVERAGE_SECONDS` (7 дней), в stderr пишется «snapshot tombstone … не подтверждён новым снимком». Карта с обеими командами, известными базе, но отсутствующая в корпусе снимков при `result_ts <= cutoff`, по-прежнему отклоняется намеренно (design, не баг) — 18.09.2026 так были вручную убраны из ledger карты Uralan (EPL) с бэкапом.
