...
- **Late-ставка не уходит против late-модели (`_late_win_model_reject_for_delivery`, 30.08.2026).** Late-модель (`base/late_win_model.py`, E-240 — обучена только на драфте карт >= 36 минут) печатается в панель строкой `🕑 Late ML-модель: <сторона> <уверенность>%`. Если сторона модели противоположна стороне ставки, а сторону ставки задаёт late-звезда без подтверждения same-sign early (`_is_late_driven_context` — та же популяция, что у late-гейта), отправка запрещена: reason `late_model_against`. Гейт стоит в единой точке доставки `_deliver_and_persist_signal`, сразу после гейта предматчевой модели, и покрывает immediate, delayed watcher и спекулятив разом. Матч НЕ закрывается (`add_url` не зовём). **Молчание late-модели блокирует ставку** (fail-closed): late-ставка уходит ТОЛЬКО когда модель явно назвала сторону таргета, иначе reason `late_model_missing`. Цена решения названа прямо: пропажа артефакта модели гасит весь late-поток, а не открывает его — дороже в объёме, дешевле в убытке; считать по `runtime/prematch_model_eval.jsonl` (`late_side`, `late_error`). Откат — `BET_REQUIRE_LATE_WIN_MODEL=0`.

**Итоговое правило late-ставки (30.08.2026).** Ставка исключительно на late уходит при ОДНОВРЕМЕННОМ выполнении четырёх условий: (1) игровое время >= `LATE_PUB_COMEBACK_TABLE_START_MINUTE` (31:00); (2) late-гейт пройден — `late_star_hit_count >= 2`, `late_wr_pct >= 65`, нет WR60+ star-хита противоположного знака в блоке All; (3) late-модель назвала сторону таргета; (4) `target_networth_diff >= threshold[gate_wr_level][минута]` из comeback-таблицы. Нарушение любого — отправки нет.
