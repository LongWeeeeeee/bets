...
- **Снятие вето comeback-таблицы (`_late_pub_table_veto_bypassed`, 29.08.2026).** На минимальной минуте и позже порог нетворта из таблицы перестаёт быть условием входа, если late-звезда сильная: `late_star_hit_count >= LATE_PUB_TABLE_VETO_BYPASS_MIN_LATE_HITS` (2) и `late_wr_pct >= LATE_PUB_TABLE_VETO_BYPASS_MIN_LATE_WR` (65 с 30.08.2026, до этого 70). Такой сигнал уходит сразу на 31:00. **Порог снятия вето теперь равен порогу late-гейта**, поэтому полосы «прошёл гейт, но ждёт таблицу» больше нет: таблица перестала быть условием входа для всех late-сигналов, проходящих гейт, и её роль ограничителя перешла к late-модели (ниже). Снятие вето НЕ открывает отправку раньше минимальной минуты и НЕ отменяет late-гейт: сначала гейт, потом вето. В журнале отправка помечается `late_pub_comeback_table_veto_bypassed=True` при `late_pub_comeback_table_reached=False` — порог нетворта в этом случае не достигнут. Точки применения: delayed watcher и immediate-ветка. Откат — `LATE_PUB_TABLE_VETO_BYPASS_ENABLED=0`.
- **Late-ставка не уходит против late-модели (`_late_win_model_reject_for_delivery`, 30.08.2026).** Late-модель (`base/late_win_model.py`, E-240 — обучена только на драфте карт >= 36 минут) печатается в панель строкой `🕑 Late ML-модель: <сторона> <уверенность>%`. Если сторона модели противоположна стороне ставки, а сторону ставки задаёт late-звезда без подтверждения same-sign early (`_is_late_driven_context` — та же популяция, что у late-гейта), отправка запрещена: reason `late_model_against`. Гейт стоит в единой точке доставки `_deliver_and_persist_signal`, сразу после гейта предматчевой модели, и покрывает immediate, delayed watcher и спекулятив разом. Матч НЕ закрывается (`add_url` не зовём). **Молчание late-модели ставку не блокирует** — в отличие от предматчевого гейта, где `model_missing` блокирует: отказ late-модели молчаливый по построению (нет артефакта, незнакомый герой, неполный драфт), и трактовать его как «против» значило бы гасить весь late-поток при пропавшем артефакте. Цена: по панели молчание не отличить от согласия, считать его надо по `runtime/prematch_model_eval.jsonl` (`late_side`, `late_error`). Откат — `BET_REQUIRE_LATE_WIN_MODEL=0`.
