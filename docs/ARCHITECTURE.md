...
После расчёта полного draft payload и pre-map ELO кандидат `early_output` WR60 с минимум двумя STAR-hit записывается в JSONL shadow schema `team_kills27_shadow.v1`. Frozen logistic artifact `ml-models/team_kills27/team_kills27_shadow.json` оценивает вероятность того, что target team сделает минимум 27 убийств.

Модель имеет 93 входа: прежние 90 draft/ELO-признаков (`nw_hit_count`, `nw_max_wr`, все шесть метрик блоков `early_nw`, `early_win`, `late`, `all`, их absolute/agreement/cross-block признаки и target-aligned ELO) плюс `roster_patch_games`, `roster_patch_mean_kills`, `roster_patch_ge27_rate`.
...
Опциональный изолированный sender применяет artifact probability threshold и минимальный NW WR. Ручных порогов по sample size или mean kills нет: patch-roster статистика является частью модели. Roster-проверка fail-closed только когда сам источник признаков недоступен; валидный источник без подходящих roster/patch-карт не блокирует отправку, и отсутствующие признаки обрабатывает frozen median imputer. Artifact threshold равен break-even для коэффициента 1.8 (`1 / 1.8 = 55.56%`) и выбран вместе с regularization только на old chronological validation; forward оставался untouched.

Telegram-сообщение показывает latest-patch roster sample, среднее, медиану и долю 27+. Антидубликат нормализует меняющиеся DLTV poll/score keys до стабильного Dota `match_id` и до HTTP-вызова атомарно фиксирует persistent at-most-once claim; default sent-log — `runtime/team_kills27_telegram_sent.jsonl`. Shadow-аудит и основной STAR/stake/bookmaker dispatch от этого контура не зависят. Имена env/модуля пока сохраняют legacy prefix `TEAM_KILLS25_*`/`team_kills25_shadow.py`.
