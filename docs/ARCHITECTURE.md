...
- Неизвестная команда при разборе матча автоматически онбордится в tier2 (`_ensure_known_team_or_add_to_tier2` → `_append_team_to_tier2_file`). С 02.09.2026 запись уходит в JSON-overlay `base/id_to_names_dynamic_tier2.json` (env `TIER2_DYNAMIC_ONBOARDING_PATH`, модуль `base/tier_dynamic_overlay.py`); читатели (`_get_team_tier`, `_find_known_team_ids_by_name`) применяют его поверх `tier_two_teams` той же семантикой слияния. До этого рантайм дописывал Python-блоки прямо в отслеживаемый `id_to_names.py` (611 блоков на serv1 — вечно грязный файл и конфликты при pull). Legacy-блоки переносятся в overlay инструментом `base/tools/migrate_tier2_onboarding.py` ПЕРЕД любой очисткой файла от блоков; ночная цепочка `rebuild_prematch_snapshot.sh` забирает с прода оба файла, чтобы `build_team_org_aliases.py` видел новые теги.
...
- **Как матч вообще попадает в tier 3** (`_classify_tier_three_sides`, cyberscore ~22172; результат читает `check_head` ~36245). Три способа, и во всех трёх неизвестная сторона **не** онбордится в tier2:
  ...
  2. лига в `TOURNAMENT_LEAGUE_ID_TIER_GATED_ALLOWLIST` (сейчас множество ПУСТОЕ; 09.09–18.09.2026 в нём был 10877) **и хотя бы одна сторона уже известна как tier1/tier2**. 10877 — общий ежедневный тикет площадки Challengermode, на котором живут и открытые квалы BLAST Slam, и чужие турниры, а название Valve (`Challengermode Daily Tournaments`) не содержит ни одного токена allowlist'а. 26.08.2026 тикет впускали безусловно, 06.09.2026 закрыли из-за авто-онбординга чужих команд, 09.09.2026 вернули условно (E-267), 18.09.2026 сняли и условный допуск: карта Radiant [0] vs Team Zhir1t (match 9005102783) прошла гейт по team_id Zhir1t, который сидит в tier2 лишь по legacy-блоку авто-онбординга в `id_to_names.py`, и ставка ml_dispatch ушла на безымянный стек. Способ 2 сейчас спит — до нового id в множестве. На открытых квалах Valve часто не отдаёт ни `team_id`, ни `team_name` — обе стороны приходят как `None`, — поэтому сторону дополнительно опознаём по составу: теги игроков из OpenDota `proPlayers` с порогом консенсуса `GATED_TICKET_MIN_TIER12_PLAYERS` (4 из 5). Теги отвечают ТОЛЬКО на вопрос «есть ли здесь команда tier1/2»: имена сторон из них не строятся, потому что тег OpenDota устаревает. Настоящие имена и team_id достаёт `_resolve_sourcetv_bridge_identity` с карточки CyberScore по ТОЧНОМУ Valve `match_id` (`id_steam`), без сопоставления по названиям;
...
- **Обычная ставка не уходит против late-модели (`_late_win_model_reject_for_delivery`, 30.08.2026, E-241).** Late-модель (`base/late_win_model.py`, E-240 — обучена только на драфте карт >= 36 минут) печатается в панель строкой `🕑 Late ML-модель (карта ≥36 мин): <сторона> <уверенность>%` (подпись в скобках с 18.09.2026; `_LATE_WIN_MODEL_PANEL_RE` читает сторону и без неё — старые delayed-записи). Если строка есть и сторона модели противоположна стороне ставки, отправка запрещена для любой обычной ставки (late/early/all): reason `late_model_against`. Гейт стоит в `_deliver_and_persist_signal`, сразу после гейта предматчевой модели. Матч НЕ закрывается (`add_url` не зовём). **Молчание late-модели** (`late_model_missing`) по-прежнему только у late-driven (`_is_late_driven_context`): late-ставка уходит ТОЛЬКО когда модель явно назвала сторону таргета. У early/all отсутствие строки не «против». Откат — `BET_REQUIRE_LATE_WIN_MODEL=0`. **Третья причина, `late_model_target_unknown` (01.09.2026).** Раньше при неизвестной стороне ставки (не пришёл `stake_multiplier_context`) гейт молча пропускал — и на пути `star_signal_sent_now_prematch_model`, единственном «СТАВКА НА <team> x<mult>» без контекста, гейта не было вовсе: из 68 обычных ставок, отправленных 30.08-01.09.2026, 4 ушли при late-модели ПРОТИВ таргета, все четыре оттуда. Теперь путь передаёт контекст, а гейт при неизвестном таргете запрещает отправку. Сторона модели берётся из строки панели, а при её отсутствии — из `stake_multiplier_context["late_model_side"]` (пересобранный текст delayed-записи).

**Ветка `no_account_no_org` — явный отказ с 18.09.2026 (`base/win_model_veto.py`).** Эта ветка предматчевой модели (7 колонок, офлайн-качество 0.6852) никогда не входила в `_PREMATCH_BET_BRANCHES` (`full,no_org`), но до 18.09.2026 `_prematch_index` всё равно отдавала по ней индекс как по полноценной оценке — 52 из 497 карт 17-18.09 ушли с `bet=True` при 0.62-0.66 без учётных данных игроков/организации. Теперь `_prematch_index` возвращает `None` (как отказ модели), журнал получает `bet=False, reason="no_account_no_org_blocked"`: ни `ml_win_index`, ни вето, ни star-поддержки, гейт доставки видит `model_missing`. Откат — `PREMATCH_ML_NO_ACCOUNT_NO_ORG_BET=1`.
...
С E-263 историческое обучение применяет исход только после `startDateTime + durationSeconds`; равное время завершения не допускает исход в прогноз текущего старта. Неизвестная длительность даёт пропуск обновления. Live применяет подтверждённый исход по времени наблюдения. Календарь ELO и сортировщика архива общий; 7.41–7.41e используют точные моменты официальных объявлений Steam. Текущие tier-priors сохранены, будущие участники лиги больше не используются.

Снимок должен явно соответствовать версии replay и календарю; pin не отменяет миграцию. Несовместимый snapshot отклоняется до явного offline rebuild. Подпись базы для runtime-state/progress/delta включает точную историю, поэтому совпадения последних timestamps недостаточно. Проверки стоят и в Telegram, и в быстрых путях чтения ELO для ML.

Ребейз живого ledger (`rebase_runtime_model_state`, `_applied_entry_for_rebase`/`_pending_entry_for_rebase`) доказывает membership старой live-карты в новом снимке тремя способами СТРОГО по порядку — карта с id- или pair-доказательством никогда не понижается до третьего правила: (1) точный `match_id` в `meta.recent_completed_match_ids`; (2) при промахе — team-pair {radiant_team_id, dire_team_id} + timestamp в допуске (`meta.recent_completed_match_keys`, по умолчанию 3 ч, ключ `[match_id, radiant_id, dire_id, timestamp]`) через `_snapshot_find_team_pair_key`; один ключ снимка покрывает НЕ БОЛЕЕ ОДНОЙ live-карты — общий `consumed`-set (заведён в `rebase_runtime_model_state`, разделяется applied+pending) резервирует найденный ключ, applied-карты обрабатываются в хронологическом порядке `result_timestamp`, чтобы более ранняя карта серии не осталась без ключа при двух картах одной серии близко по времени; (3) если пара тоже не нашлась — «обе команды неизвестны базе»: `known_team_ids` = статический `TEAM_ID_TO_ORG_KEY` (разбор `roster_ratings`/`player_current_org` снимка, т.к. сырого `team_id` в `model_state` нет) ∪ сырые id из `recent_completed_match_keys` (реестр неполон для команд корпуса вне `base/id_to_names.py`) — карта «replay»/«pending» БЕЗ порядкового гейта; при известности хотя бы одной стороны действует прежний order-gated replay/raise. E-294 round 3 (17.09.2026): 92 covered / 5 replay / 59 replay_unknown_teams / 3 error из 159 — остаток либо третья подряд карта серии без своего ключа, либо ровно одна известная команда. Покрытые/replayed карты сжимаются в компактный `_snapshot_covered_tombstone` (без полного `match_record`) при атомарной замене runtime-файлов; байт-точный оригинал держится как бэкап до успешной записи, откат восстанавливает его при сбое. Отдельный гейт (сразу после `full_model_state(snapshot)`) блокирует саму перебазировку при незавершённой live-работе только если различаются хэши `_model_config_signature(model_state.config)` старого и нового состояния — а не `meta.model_config_signature` снимка (это хэш всей истории матчей, «_rating_history_signature», меняющийся по конструкции на каждой ночной пересборке).

Tombstone, доказанный правилом (2) team-pair, хранит фактически использованный ключ снимка (`snapshot_covered_match_id`). Следующий ребейз пытается подтвердить его тем же ключом в новом снимке; если не находит — tombstone не отбрасывается как ошибка, а остаётся до истечения окна `SNAPSHOT_RECENT_RESULT_COVERAGE_SECONDS` (7 дней), в stderr пишется «snapshot tombstone … не подтверждён новым снимком». Карта с обеими командами, известными базе, но отсутствующая в корпусе снимков при `result_ts <= cutoff` — 18.09.2026 так вручную убирались из ledger карты Uralan (EPL) с бэкапом, когда это ещё было жёстким отказом. С bbd0664f/04b8252c поведение по умолчанию другое: такая карта replay'ится после среза (действие `replay_late`, stderr «… replay после среза, порядок ELO-событий нарушен»), т.к. отказ не восстанавливает порядок событий, а лишь морозит прод на устаревшей базе; старый жёсткий отказ включается через `LIVE_ELO_REBASE_STRICT_ORDER=1`. Отдельно replay схлопывает «alias-твинов» — две ledger-записи с одним `match_id`, той же парой команд и `result_timestamp` в пределах `LIVE_ELO_ALIAS_TWIN_WINDOW_SECONDS` (default 600 c) — к модели применяется только первая, сама запись ledger сохраняется для обеих.

`register_live_map_context` принимает необязательный `observed_game_time`; в sourcetv-режиме доигранная карта ~15 мин видна повторно под alias match_id серии, и без гейта применялась дважды (E-294 follow-up, 29/162 серий в проде). Дубль отсекается только когда `observed_game_time` достиг `LIVE_ELO_ALIAS_DUPLICATE_MIN_GAME_TIME_SECONDS` (default 600 c) — следующая карта серии регистрируется на драфте/раннем гейм-тайме и от эха не отличима по одному match_id. `register_live_map_context` и `finalize_live_series_from_scores` обновляют рейтинги по завершённым картам. Предматчевая ML сохраняет `ELO/models.py::prematch_lineup_summary` и обучающий контракт E-173 (`team_id=None`, имя, TIER3, позиции вместе с account ID, полный `team_strength`, разница /400). Основной `get_matchup_summary` переведён на независимый K24, описанный ниже. Время расчёта проходит из `startDateTime` через `win_model_veto.elo_evaluation_timestamp`; без корректной даты используется текущее время. Неполный или невалидный состав даёт `None`; рейтинг из kills-priors больше не подставляется.

После E-297 основной `get_matchup_summary` использует `ELO/models.py::k24_lineup_summary`: среднее пяти независимых `player_k24`, начальный рейтинг 1500, K=24, шкала 400. Telegram показывает «ELO состава (K24)» и теоретическую вероятность Elo. Это не итоговая вероятность ML: `prematch_lineup_summary` и обученные признаки `hybrid_strength`, `elo`/`opp_elo` сохраняют прежнюю семантику; модель не переобучается. Диагностическая функция `last_model_elo` также сохранена. Исторический `build_matchup_summary_from_snapshot` остаётся отдельным аналитическим API. Базовый снимок строится из `pro_heroes_data/json_parts_split_from_object`; hybrid-линия состава по-прежнему требует минимум 4 общих игроков.

K24 обновляется только исходами завершённых карт, без tier/roster/side-bias/decay поправок hybrid. История обновлений за 14 дней позволяет исключить из прогноза все исходы с `result_timestamp >= timestamp` начала карты. Запрос до доступного покрытия, старое состояние без K24-схемы или повреждённое выбранное live-состояние дают недоступный ELO, а не запасной hybrid. Смена replay-подписи требует явной offline-пересборки; array sidecar и live delta имеют новые версии. Для старого результата внутри покрытия K24 отменяет последующие обновления и пересчитывает их после вставки; hybrid не пересчитывается. Индекс membership также хранит 14 дней, tombstone по-прежнему 7 дней. Если восстановить порядок K24 нельзя, ребейз отказывает до записи независимо от разрешённого legacy `replay_late`. Основные ранги также используют K24; snapshot-ранг не отдаётся историческому запросу до/на время снимка. Подробности: [основной K24](elo-k24-primary.md).

С 25.09.2026 (E-338, приказ владельца «Сразу заменить K24») `get_matchup_summary` отдаёт вариант A. Это K24 с тремя множителями к K: длительность карты (1981 с / длительность, клип 0,6–1,6, пол 600 с), затухание при ожидаемой победе и ускорение новых игроков (2/(1+игр/30)). Вне отбора на 160 200 картах за 27 месяцев точность выше на 0,924 п.п. [0,796; 1,057], лучше в 26 месяцах из 27. На картах команд бота выбор фаворита почти тот же, улучшаются log loss и отбор по порогу 50 пунктов. A считается параллельно с K24 в том же состоянии, правила доступности и покрытия у них общие. Откат — `ELO_SERVED_COMPOSITION=k24`. Снимок без A-состояния отдаёт K24. Живая длительность берётся из Stratz; если её нет, множитель 1, а ночная пересборка пересчитывает всё по корпусу. Подробности: [вариант A](elo-variant-a.md).

Кэши перечитывают заменённый снимок и живые обновления; блок `hybrid_*` больше не держит первую загруженную модель до рестарта. Предпросмотр не меняет счётчики. ELO по-прежнему влияет на запрет доставки x0.5, tempo и правила, использующие место команды в таблице рейтингов; удалён только прежний ELO-гейт внутри множителя ставки. Поэтому унификация меняет эти решения, даже если вход ML сохранён. Проверки и границы вывода: [E-261](experiments/E-261-unified-live-elo-contract.md).

Тот же снапшот содержит kills-history schema v2: `team_kills_history_by_team_id` хранит для каждого Dota team ID до 100 последних карт с `match_id`, timestamp, пятью player IDs, числом убийств команды и source patch. Patch извлекается из split-файлов `<patch>_part<N>.json`, а `meta.team_kills_history_latest_patch` берётся у самой поздней датированной карты с известным patch. Kill arrays из raw Stratz суммируются загрузчиком; карты без валидных kills/пятёрки игроков пропускаются, архивные дубликаты удаляются по `match_id`. JSON заменяется атомарно, а отсутствующая либо не-v2 kills-history пересобирается при `rebuild_if_missing=True`, если версия replay актуальна.
...
Тёплый снимок + weak-corroboration (E-270, 10.09.2026): холодный съём ленты отдаёт shell (149 символов) и обязан отклоняться (`_winline_overview_payload_looks_like_feed`, иначе затирает хороший снимок и join мёртв); join передаёт в `card_fn` DOM снимка (плоский текст не делит соседние live-карточки под одним заголовком). Weak-хинт (топ-тег 3/5 при единственном лидере, флаг `weak`, `display` для матчера) едет в мост только с гейтового тикета и открывает league-фильтр/ team_id-гейт лишь в паре с живой карточкой (лига карточки вне denylist, вторая сторона — точное имя из моста). Кейсы: PlayTime–Yellow Submarine (GC 8991598414) идёт целиком, Inner Circle–kalamychata на момент внедрения upcoming (DLTv 427962, на GC отсутствует).
...
Решение владельца 20.09.2026 (`_lane_elo_release`): ожидание также снимается,
если ELO таргета строго выше соперника И (ML Laning ≥0.55 за таргет ИЛИ
`|Ctx.lane_adv_dict|` ≥8 в его пользу) — env `ML_DISPATCH_LANE_ELO_RELEASE`
(rollback `=0`), `ML_DISPATCH_LANE_ELO_RELEASE_LANE_CONF` (0.55),
