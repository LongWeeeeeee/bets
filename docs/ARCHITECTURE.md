...
- `synergy_duo`/`synergy_trio` могут выводиться в Telegram-блоках, но **не** входят в `STAR_SIGNAL_METRICS` и не создают самостоятельный STAR-hit/dispatch. Новая trio-агрегация и WR-разметка этого не меняют.
