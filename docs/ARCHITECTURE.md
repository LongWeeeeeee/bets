...
`register_live_map_context` принимает необязательный `observed_game_time`; в sourcetv-режиме доигранная карта ~15 мин видна повторно под alias match_id серии, и без гейта применялась дважды (E-294 follow-up, 29/162 серий в проде). Дубль отсекается только когда `observed_game_time` достиг `LIVE_ELO_ALIAS_DUPLICATE_MIN_GAME_TIME_SECONDS` (default 600 c) — следующая карта серии регистрируется на драфте/раннем гейм-тайме и от эха не отличима по одному match_id. `register_live_map_context` и `finalize_live_series_from_scores` обновляют рейтинги по завершённым картам. `get_matchup_summary` и предматчевая ML используют общий `ELO/models.py::prematch_lineup_summary`: обучающий контракт E-173 (`team_id=None`, имя, TIER3, позиции вместе с account ID, полный `team_strength`, разница /400). Время расчёта проходит из `startDateTime` через `win_model_veto.elo_evaluation_timestamp`; без корректной даты используется текущее время. Неполный или невалидный состав даёт `None`; рейтинг из kills-priors больше не подставляется.

Telegram показывает один «ELO состава (как в ML)» и его теоретическую вероятность по шкале Elo. Это не итоговая вероятность ML: внутри модели также остаются обученные признаки `elo` K24/`opp_elo`, драфт и статистика игроков. Диагностическая функция `last_model_elo` сохраняет прежнюю семантику для журналов. Исторический `build_matchup_summary_from_snapshot` остаётся отдельным аналитическим API. Базовый снимок строится из `pro_heroes_data/json_parts_split_from_object`; продолжение линии состава требует минимум 4 общих игроков.
