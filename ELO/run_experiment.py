...
def _default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "output" / "default_run"


def main() -> None:
    parser = argparse.ArgumentParser(description="Run tier-aware Elo experiments for Dota 2 pre-draft prediction.")
    ...
    baseline_report = run_online_evaluation(
        model=SimpleTeamEloModel(config=baseline_config),
        matches=matches,
        config=evaluation_config,
    )
    hybrid_report = run_online_evaluation(
        model=HybridPlayerRosterEloModel(config=hybrid_config),
        matches=matches,
        config=evaluation_config,
    )

    tier_match_counts: dict[str, int] = {}
    for tier in LeagueTier:
        tier_match_counts[tier.value] = sum(1 for match in matches if match.derived_league_tier == tier)
    ...
    full_report = {
        ...
        "dataset_summary": {
            **load_summary,
            ...
            "matches_by_derived_tier": tier_match_counts,
        },
        ...
        "models": {
            "simple_team_elo": baseline_report,
            "hybrid_player_roster_elo": hybrid_report,
        },
    }
    ...
    league_diagnostics = [
        ...
        for info in sorted(
            league_info.values(),
            key=lambda info: (info.derived_tier.value, -info.team_count, info.league_name.lower(), info.league_id),
        )
    ]

    with (args.output_dir / "report.json").open("w", encoding="utf-8") as fh:
        json.dump(_to_json_ready(full_report), fh, ensure_ascii=False, indent=2)
    with (args.output_dir / "league_diagnostics.json").open("w", encoding="utf-8") as fh:
        json.dump(league_diagnostics, fh, ensure_ascii=False, indent=2)
