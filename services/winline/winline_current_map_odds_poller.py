...
# Путь намеренно остаётся runtime/: до переноса модуль лежал в runtime/, и
# .parent давал именно runtime/ (ср. base/cyberscore_try.py:471 — замеры взяты
# «по runtime/winline_odds_history.jsonl»). Это рантайм-состояние, ему не место
# в исходниках, а смена пути осиротила бы уже накопленную историю.
