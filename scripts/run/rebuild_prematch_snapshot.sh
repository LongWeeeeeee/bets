...
CHECK
  ...
  # kills-v3 state для shadow-модели B панели (E-329). Не фатален: панель и
  # ставки от него не зависят. Собирается только если модель B лежит локально;
  # на serv1 уезжает только при маркере runtime/kv3_state_deliver.on. Процесс
  # прода перечитывает файл сам по mtime/размеру — рестарт не нужен.
