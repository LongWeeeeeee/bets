...
#
# Не снижено до 3, хотя обрыв качества измерен именно там (см. _SNAPSHOT_WARN_DAYS):
# полосы в `runtime/artifacts/misc/audit_live_path.md` немонотонны и малы —
# 0.7231 при <=3 сут (n=549), 0.6879 при 3-7 (n=630), 0.7108 при 7-14 (n=906),
# 0.7085 при 14-30 (n=1873). Этот порог отвечает на «ночная пересборка вообще
# жива?», а не на «снимок свежий?»; за второе отвечает предупреждение ниже.
# Узкое место — не порог, а то, что боевой расчёт 33 из 35 признаков расходится
# с обучающим (корреляции 0.23-0.66), и веса под него не переобучены.
_SNAPSHOT_MAX_AGE_DAYS = float(os.getenv("WIN_MODEL_SNAPSHOT_MAX_AGE_DAYS", "30"))
