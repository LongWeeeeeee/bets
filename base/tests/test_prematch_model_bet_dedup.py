...
поэтому это уже per-map). Раздел разных матчей даёт match_id, а не команды.
Запасного пути на «сырой» суффиксный ключ больше нет — попадание в него
означает возврат старого бага.

d7ad05f (02.09.2026, non-blocking review note): суффикс `|map{N}`,
добавлявшийся к ключу через `_bookmaker_infer_map_num(live_league)`, снят.
Он был лишним и опасным: номер карты резолвится не в каждом цикле опроса
(зависит от свежести GC-счёта серии в `live_league`), а один опрос без него
давал ДРУГОЙ ключ для ТОЙ ЖЕ карты — та же дыра, что и с сырым `.N`-суффиксом,
только по другой причине. На боевом 206-строчном ledger serv1 у каждого
base URL — ровно один map_num, так что суффикс ничего не разделял.
...
def test_same_map_dedup_with_series_wins_shaped_live_league(
    ...
    список явных ключей `_bookmaker_infer_map_num`). Ключ дедупа больше не
    зависит от разбора `live_league` вовсе (d7ad05f), но регрессия остаётся:
    эта форма не должна ронять `_try_dispatch_prematch_model_bet` целиком и
    обязана держать ровно одну отправку на карту, как и любая другая форма."""
    row0, row1 = _load_dup_pair_rows()
