...
def test_team_elo_min_rating_guard_rejects_target_below_1500() -> None:
    decision = runtime._team_elo_min_rating_guard_decision(
        team_elo_meta={
            "radiant_base_rating": 1499.0,
            "dire_base_rating": 1540.0,
        },
        target_side="radiant",
    )
    ...
    assert decision["target_rating"] == pytest.approx(1499.0)
    assert decision["min_rating"] == pytest.approx(1500.0)


def test_signal_rejected_when_target_team_elo_below_1500(monkeypatch) -> None:
    case = BranchScenario(
        name="target_team_elo_below_1500_reject",
        game_time_seconds=12 * 60,
        target_side="radiant",
        target_networth_diff=1200,
        has_early_star=True,
    ...
    monkeypatch.setattr(
        ...
        "_build_team_elo_matchup_summary",
        lambda *_args, **_kwargs: {
            "radiant": {"rating": 1499.0, "base_rating": 1499.0, "lineup_used": True},
            "dire": {"rating": 1550.0, "base_rating": 1550.0, "lineup_used": True},
            "radiant_win_prob": 0.49,
            "dire_win_prob": 0.51,
            "elo_diff": -51.0,
            "source": "test",
        },
    ...
    assert result.queued_payload is None
    assert result.add_url_calls
    assert result.add_url_calls[-1]["reason"] == "star_signal_rejected_team_elo_below_1500"
    details = result.add_url_calls[-1]["details"]
    assert isinstance(details, dict)
    assert details["target_side"] == "radiant"
    assert details["target_rating"] == pytest.approx(1499.0)
    assert details["min_rating"] == pytest.approx(1500.0)
