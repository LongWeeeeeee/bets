...
@pytest.mark.parametrize("minimum", [1000, 1500])
def test_target_minimum_is_inclusive_directional_and_separate_from_entry(minimum):
    sys.path.insert(0, str(study.ROOT / "scripts/ops"))
    from research_lane_minute_schedule import target_hit, replay
    data = {"nw10": np.array([minimum - 1, minimum, -minimum, 1 - minimum, 0, np.nan]),
            "dispatch_target": np.array([1, 1, -1, -1, 1, 1])}
    assert target_hit(data, minimum).tolist() == [False, True, True, False, False, False]
    ...
    assert take.all() and target_hit(data, minimum)[take].sum() == 2
