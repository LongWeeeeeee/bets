...
# --- "Any one of the three" wins, for both F and U ------------------------

def test_any_one_of_three_win_models_confirms_the_favorite():
    for model_name in ("late", "all", "early_win"):
...
def test_any_one_of_three_win_models_confirms_the_underdog():
    for model_name in ("late", "all", "early_win"):
...
def test_kills_window_absent_without_an_open_window():
    ctx = base_ctx(elo_radiant=1400.0, elo_dire=1500.0,
                    ...
                    kills_windows_open=[])
    result = evaluate(ctx, cfg())
    kills = {d.market for d in result.decisions}
...
def test_config_from_env_defaults_when_unset():
    ...
    assert result.win_models == ("late", "all", "early_win")
