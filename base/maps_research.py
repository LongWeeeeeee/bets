...
#     лимите 300 000, то есть один матч стоит около 34 546. На живом сборе —
#     8.0 карт на запрос, ошибок ноль.
# Прежняя запись «один матч = один запрос» была неверной и стоила
...
# НЕ ИСПОЛЬЗУЕТСЯ. Заготовка перехода: вызовов нет ни одного, во всём дереве
# единственное вхождение имени — эта строка (проверено 2026-08-27). Рабочей
# остаётся PLAYBACK_QUERY выше.
#
# Появилась при сведении serv1 с main: git слил две независимо написанные
# копии секции playbackData БЕЗ конфликтных маркеров, потому что в базе
# слияния этой секции не было вовсе. Замысел сохранён намеренно — четыре
# новых поля (timeDead, assist, positionX, positionY) добавлены В КОНЕЦ
# записи смерти именно для того, чтобы старые читатели (ev[0..6])
# продолжали работать на смешанном корпусе. Переход не доведён.
#
# Чтобы включить: заменить PLAYBACK_QUERY на PLAYBACK_QUERY_V2 в месте
# запроса и научить читателей ev[7..10]. Если переход решено не делать —
# удалить обе эти константы одним коммитом, история сохранит.
