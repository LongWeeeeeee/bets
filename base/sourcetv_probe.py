...
def _keyword_candidate_league_ids():
    ...
    for lid, nm in _LEAGUE_NAMES.items():
        ...
        if (
            ...
            out.append(lid)
    # Явно разрешённые id опрашиваем всегда, вне окна «текущей эры»: платформенный
    # тикет Valve может быть зарегистрирован задолго до KW_RECENT_FLOOR, и тогда
    # прямого опроса по нему не было бы вовсе — только надежда на широкий снимок
    # (0), который на пике режется по числу игр.
