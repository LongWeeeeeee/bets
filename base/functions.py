...
# Перемер по прод-критерию — точности в ТОПЕ отбора, как требует правило 1
# журнала («мерить на прод-отборе, а не по AUC») — даёт другой ответ. Про-карты,
# цель «победа карты», топ-600 у early и топ-400 у late/post_lane:
#   порог      5      10      30      50     100     200
#   early   59.3%   59.2%   58.2%   58.8%   57.2%   57.5%
#   late    63.8%   63.2%   62.8%   63.8%   62.5%   59.0%
#   post    56.8%   57.8%   58.5%   58.8%   57.8%   56.2%
# Сотня хуже плато 5-50 во ВСЕХ трёх блоках (-1.6 / -1.3 / -1.0), двести плохо
# везде. 50 — единственная точка, оптимальная сразу для трёх.
#
# Порог режет всего две ячейки из двадцати пяти на карту (22.8 против 24.8), но
...
STAR_SIGNAL_METRICS = frozenset({
    ...
    # винрейтом 68.9% при базе 57.5% (z=2.7), и по деньгам +16.5% ROI на
    # фаворитах против +0.8% у текущего отбора. В early и late порогов НЕТ:
    # там замер показал шум либо минус (E-73, E-76).
    'ml_win_index',
...
#   без 2v2 вовсе           93.2 / 89.5 / 83.1
# Плоская кривая у первого варианта — подпись шума: «самые уверенные» предсказания
# приходят из двухигровых ячеек и ничем не лучше остальных.
#
# При пороге 30 ветка 2v2 срабатывает на 0.4% линий, то есть каскад практически
# всегда доходит до надёжных ключей. Вступает в силу при деплое.
LANE_2V2_MIN_GAMES = _lane_min_games_env("LANE_2V2_MIN_GAMES", 30)
# Ветка 2v1/1v2 ВЫКЛЮЧЕНА (E-66, 09.08). Замер: специфичный ключ проигрывает
# среднему четырёх пар 1v1 при ЛЮБОМ размере ячейки, включая сто и больше игр:
#   2v1: 2-3 игры -7.4 | 4-9 -5.9 | 10-29 -3.8 | 30-99 -2.2 | 100+ -1.4 п.п.
#   1v2: 2-3 -6.4 | 4-9 -5.0 | 10-29 -2.8 | 30-99 -1.7 | 100+ -1.5 п.п.
# Даже одиночная пара со 100+ играми хуже среднего четырёх таких же пар на 2.0.
# Дело не в пороге, а в конструкции: специфичность покупается дисперсией, и обмен
# всегда убыточный. Порог поднят до заведомо недостижимого, чтобы каскад доходил
...
#   точн  66.41 66.43 66.44 66.44 66.45 66.48 66.49 66.50   (±0.17)
# Весь диапазон двигает точность на 0.09 п.п. — меньше одной сигмы. Разница
# между 1.7 и 3.0, выглядевшая на про как +0.8 п.п. на топ-500, здесь равна
# 0.04 п.п.: то был шум малой выборки.
#
# Парный тест (Макнемар, вес 0 против 3) показывает, что слой всё же не пустой:
# вердикт меняется у 428 линий из 74 784, из них ПОЧИНЕНО 240 против СЛОМАНО
# 188, перевес +52, z = +2.51. Информация есть и знак верный, но величина —
# +0.07 п.п. точности, то есть операционно ноль. Слой оставлен потому, что он
# измеримо не вредит, а не потому, что что-то даёт.
