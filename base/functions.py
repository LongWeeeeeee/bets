...
# 30 -> 100 (05.08) -> 30 (09.08). Подъём до сотни опирался на AUC. Перемер на
# ПЕРЕСОБРАННЫХ словарях по прод-критерию (точность в топе отбора на про-картах,
# цель «победа карты», топ-600 у early и топ-400 у late):
#   порог      5      10     30      50     100     200
#   early   60.7%  60.7%  60.7%  60.5%   60.3%   60.0%
#   late    53.8%  54.0%  54.8%  54.2%   53.2%   52.8%
# Кривая одногорбая с максимумом на 30, сотня стоит на спадающей ветке (-1.6 п.п.
# у late). Та же ошибка, что была у COUNTERPICK_1VS1_MIN_MATCHES: выбор по AUC
# вместо прод-отбора.
...
# 100 -> 50 (09.08). Выбор сотни опирался на AUC по всем картам; по прод-критерию
# ответ другой, и пересборка словарей его подтвердила — про-карты, топ-400 у late:
#   порог      5      10     30      50     100
#   late    54.5%  54.5%  54.8%  55.2%   54.8%
# early при этом плоский (63.8% на всех порогах), то есть цена нулевая.
...
# ролевому семейству врага с весом k. 0 = прежняя жёсткая отсечка (по умолчанию).
#
# ВЫКЛЮЧЕНО. Механика работает и по качеству неотрицательна (ПРО: Late AUC
# 0.6323 -> 0.6370, All 0.5738 -> 0.5730), но включение ломает два контракта:
#   * возвращаемые games = n + k утекают в диагностические поля *_games,
#     которые должны показывать реальный размер выборки (pos1_vs_pos1_games
#     68 -> 168) и уходят в Telegram;
#   * гейт покрытия становится вакуумным не только для cp1vs1, но и для
#     производных метрик, терявших на тонких данных защиту (pos1_vs_pos1
#     переставал быть None).
# Чтобы включить, нужно развести две роли числа games — гейт покрытия и
# диагностику — и только потом поднимать флаг. Выигрыш (+0.005 AUC в Late,
# -0.001 в All) в пределах шума и не оправдывает поломку контракта.
...
STAR_SIGNAL_METRICS = frozenset({
    ...
    # Драфтовая ML-оценка победы как ЗВЁЗДНАЯ метрика. Порог задан ТОЛЬКО для
    # all_output (10 на WR60): там вето стоит на нуле и уже сняло все несогласия,
    # поэтому метрика может лишь ДОБАВЛЯТЬ карты — на про это +119 карт с
    # винрейтом 68.9% при базе 57.5%. В early и late порогов НЕТ.
    'ml_win_index',
    # pos1 против pos1: early_output — 7/11/15 на WR60/65/70 (на своей цели,
    # маркере раннего перевеса по нетворту: 64.9% / 68.4% / 69.4% на 1229/656/324
    # про-картах); mid_output — только 8 на WR60 (58.8% на 434 картах популяции
    # is_late_match). Выше 60 для late порога НЕТ намеренно: метрика там сильна
    # в середине и выдыхается к хвосту, |v|>=14 даёт 50.8%. В all_output порогов
    # нет. См. docs/EXPERIMENTS.md E-71.
    'pos1_vs_pos1',
})
...
# 2 -> 30 (E-66, 09.08). Порог служит двум местам: старой ветке каскада и новому
# пятому члену ансамбля в counterpick_lanes. Прежние 2 игры ломали каскад: ветка
# срабатывала на 37.4% боковых линий, медиана игр в ячейке 2, и вердикт брался из
# монетки с тремя гранями (0/50/100%), вытесняя ключи с порогами 20 и 50.
LANE_2V2_MIN_GAMES = _lane_min_games_env("LANE_2V2_MIN_GAMES", 30)
# Ветка 2v1/1v2 ВЫКЛЮЧЕНА (E-66): специфичный ключ проигрывает среднему четырёх
# пар 1v1 при любом размере ячейки, включая 100+ игр (2v1 -1.4 п.п., 1v2 -1.5).
# Порог поднят до недостижимого, чтобы каскад доходил до counterpick_lanes.
...
# В E-66 этот слой признавался бесполезным — но входил РАВНЫМ членом простого
# среднего и разбавлял четвёрку пар. Вес решает: 2.853/6.704 = 0.43 от блока
# пар, блок пар несёт суммарный вес 4, отсюда 4 * 0.43 = 1.7.
# Проверка веса на honest holdout (74 784 линии): кривая монотонная, но плоская —
# от 0 до 6 точность растёт с 66.41% до 66.50% при ошибке 0.17, то есть весь
# диапазон меньше сигмы. Парный тест Макнемара (вес 0 против 3) значим: 240
# починенных линий против 188 сломанных, перевес +52, z = +2.51. Слой не пустой,
# но его величина — +0.07 п.п. точности.
LANE_DUO_VS_SOLO_WEIGHT = float(os.getenv("LANE_DUO_VS_SOLO_WEIGHT", "3.0"))
LANE_2V1_ENSEMBLE_MIN_GAMES = _lane_min_games_env("LANE_2V1_ENSEMBLE_MIN_GAMES", 20)
# Вес ячейки 2v2 опущен 1.0 -> 0.08 по той же пропорции. Проверено на про:
# значения 1.0, 0.08 и 0.0 дают одинаковый результат до сотой доли.
...
def counterpick_lanes(radiant_heroes_and_pos, dire_heroes_and_pos, heroes_data, lane, return_probs=False, core_support_side_lanes=False):
    ...
    def _pair_bucket(radiant_pair, dire_pair):
        """Ячейка 2v2 пятым членом ансамбля (E-66): при 30+ играх она даёт
        73.2% против 72.1% у чистой четвёрки, при 10-29 — 67.5% против 67.3%,
        а ниже десяти игр вредит. Единственный ключ, описывающий всех четверых."""
    ...
    def _duo_vs_solo_bucket(radiant_pair, dire_pair):
        ...
        Билдер пишет ОБЕ формы уже с ориентацией на радианта: «пара радианта
        против одиночки дайра» и «одиночка радианта против пары дайра». Знак не
        переворачивается вручную — ручной переворот второй половины гасил первую
