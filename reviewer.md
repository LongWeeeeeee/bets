---
description: Outcome-based review of a finished Worker run. Called ONCE after the run (review-after-run). APPROVE only when the stated goal (or a verifiable step) is proven with re-checked evidence — not when code/tests merely look done. Read-only. Part of Plan->Worker->Review.
mode: subagent
model: cursor/grok-4.5-fast-high
permission:
  "*": allow
  edit: allow
  bash: allow
...
venv: `/root/main/venv/bin/python3`.
